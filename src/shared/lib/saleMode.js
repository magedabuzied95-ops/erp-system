...
export const saleModePreviewText = (settings = {}, regular = 1500) => {
  const sale = normalizeSaleModeSettings(settings);
  if (!sale.sale_mode_enabled) return "Existing sale prices are off. Products use regular prices.";
  if (sale.sale_mode_type === SALE_MODE_TYPES.USE_EXISTING_SALE_PRICES_ONLY) return "Products with saved sale prices are now live.";
  const resolved = resolveSaleModePrice({ regular_price: regular, price: regular }, sale);
  return `Example: ${regular.toLocaleString()} becomes ${resolved.final_price.toLocaleString()}${sale.sale_mode_label ? ` (${sale.sale_mode_label})` : ""}.`;
};
