...
import InvoiceBlockView from "./InvoiceBlockView";
...
export default function OrderInvoiceCard({ order, items, invoice, className = "", compact = false, luxury = false, publicView = false, template = null, output }) {
  ...
  // The customer's public link and the in-app preview are the same component but not
  // the same document: the link has always carried the policy, the review buttons and
  // the footer, and the preview never has.
  const layoutOutput = output || (publicView ? "public" : "card");
  const tpl = invoiceTemplateForOutput(template || resolvedTemplate, layoutOutput === "public" ? "card" : layoutOutput);
  const layoutBlocks = blocksForOutput(tpl.blocks, layoutOutput);
  ...
  // Each block draws exactly what it drew when the order was fixed; only the sequence is
  // data now. The two sections that used to share one padded wrapper get their own, so
