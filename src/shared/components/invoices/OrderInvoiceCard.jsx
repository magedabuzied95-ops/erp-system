...
function InvoiceImage({ src, alt }) {
  ...
  return <img src={imageUrl} alt={alt} className="h-full w-full object-contain p-1.5" loading="lazy" />;
}
    ...
    <article dir="rtl" className={`overflow-hidden rounded-[1.75rem] border border-stone-200 bg-white text-right text-stone-950 shadow-[0_18px_50px_rgba(39,20,75,0.07)] ${className}`}>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex items-start gap-3">
            <div className="grid h-16 w-16 shrink-0 place-items-center overflow-hidden rounded-2xl border border-stone-200 bg-white">
              {data.store?.logoUrl ? <img src={data.store.logoUrl} alt={data.store?.name || "Store"} className="h-full w-full object-contain p-2" /> : <Store className="h-7 w-7 text-stone-700" />}
            </div>
            <div>
      ...
      <div className="px-5 pb-5">
        <div className="overflow-hidden rounded-2xl border border-stone-200">
          ...
          <div className="divide-y divide-stone-200">
            {data.items.length ? data.items.map((item) => (
              <div key={String(item.id)} className="grid grid-cols-[minmax(0,1.7fr)_0.7fr_0.75fr_0.8fr] items-center gap-2 px-4 py-3 text-sm sm:grid-cols-[minmax(0,1.8fr)_0.8fr_0.75fr_0.8fr_0.9fr]">
        ...
        <div className="mt-4 mr-auto w-full max-w-sm rounded-2xl border border-stone-200 bg-stone-50 p-4">
          <Summary label="المجموع الفرعي" value={formatCurrency(data.totals.subtotal)} />
          <Summary label="الخصم" value={`- ${formatCurrency(data.totals.discount)}`} />
          <Summary label="الشحن" value={formatCurrency(data.totals.shipping)} />
          <div className="mt-3 flex items-center justify-between border-t border-stone-200 pt-3 text-lg font-black">
            <span>الإجمالي النهائي</span>
            <span className="text-emerald-700">{formatCurrency(data.totals.grandTotal)}</span>
          </div>
        </div>
      </div>
    </article>
