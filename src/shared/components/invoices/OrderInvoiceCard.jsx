...
const getPaymentMethodLabel = (value = "") => {
  ...
  return labels[normalized] || (value || INVOICE_COPY.notSpecified);
};

const formatVariantDetails = (item = {}) => {
  const parts = [];
  if (item?.color) parts.push(`${INVOICE_COPY.color}: ${item.color}`);
  if (item?.size) parts.push(`${INVOICE_COPY.size}: ${item.size}`);
  return parts.join(" • ") || INVOICE_COPY.notSpecified;
};
...
export default function OrderInvoiceCard({ order, items, invoice, className = "", compact = false, luxury = false, publicView = false }) {
  ...
  const unavailable = safeLabel(t("storefront.invoice.unavailable", { defaultValue: INVOICE_COPY.notSpecified }), INVOICE_COPY.notSpecified);
  const shippingLabel = INVOICE_COPY.shipping;
      ...
      <div className={`${luxury ? "px-5 pb-6 sm:px-6" : "px-5 pb-5"}`}>
        <div className={`overflow-hidden rounded-2xl border ${luxury ? "border-slate-200/90 bg-white/72 shadow-[0_18px_55px_rgba(15,23,42,0.08)] print:border-slate-200 print:bg-white print:shadow-none" : "border-stone-200"}`}>
          <div className={`grid grid-cols-[minmax(0,1.7fr)_0.7fr_0.75fr_0.8fr] px-4 py-3 text-xs font-black sm:grid-cols-[minmax(0,1.8fr)_0.8fr_0.75fr_0.8fr_0.9fr] ${luxury ? "bg-slate-100/80 tracking-[0.08em] text-slate-500 print:bg-slate-50" : "bg-stone-50 text-stone-500"}`}>
            <div>{INVOICE_COPY.product}</div>
            <div className="hidden sm:block">{`${INVOICE_COPY.color} / ${INVOICE_COPY.size}`}</div>
            <div className="text-center">{INVOICE_COPY.quantity}</div>
            <div className={amountAlignClass}>{INVOICE_COPY.price}</div>
            <div className={amountAlignClass}>{INVOICE_COPY.total}</div>
          </div>
          <div className={`${luxury ? "divide-y divide-slate-200/80" : "divide-y divide-stone-200"}`}>
            {invoiceItems.length ? invoiceItems.map((item, index) => {
                ...
                <div key={String(item?.id ?? index)} className={`grid grid-cols-[minmax(0,1.7fr)_0.7fr_0.75fr_0.8fr] items-center gap-2 px-4 text-sm sm:grid-cols-[minmax(0,1.8fr)_0.8fr_0.75fr_0.8fr_0.9fr] ${luxury ? "bg-white/55 py-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.7)] print:bg-white print:shadow-none" : "py-3"}`}>
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className={`mt-1 truncate text-xs font-bold sm:hidden ${luxury ? "text-slate-500" : "text-stone-500"}`}>{formatVariantDetails(item)}</div>
                      {item?.sku ? <div className={`mt-1 truncate text-[11px] font-bold ${luxury ? "text-slate-400" : "text-stone-400"}`}>SKU {item?.sku}</div> : null}
                    </div>
                  </div>
                  <div className={`hidden text-sm font-bold sm:block ${luxury ? "text-slate-500" : "text-stone-500"}`}>{formatVariantDetails(item)}</div>
              ...
              <div className={`px-4 py-8 text-center text-sm font-bold ${luxury ? "text-slate-500" : "text-stone-500"}`}>{INVOICE_COPY.empty}</div>
            )}
          </div>
        </div>
        ...
        <div className={`mt-5 w-full max-w-sm rounded-2xl border p-4 ${luxury ? "border-slate-200/90 bg-slate-50/90 shadow-[0_16px_45px_rgba(15,23,42,0.08)] print:border-slate-200 print:bg-slate-50 print:shadow-none" : "border-stone-200 bg-stone-50"} ${isRtl ? "mr-auto" : "ml-auto"}`}>
          <Summary luxury={luxury} label={INVOICE_COPY.subtotal} value={formatCurrency(totals?.subtotal)} />
          <Summary luxury={luxury} label={INVOICE_COPY.discount} value={`- ${formatCurrency(totals?.discount)}`} />
          <Summary luxury={luxury} label={shippingLabel} value={formatCurrency(totals?.shipping)} />
          {totals?.exchangeMode ? (
            <>
              <Summary luxury={luxury} label="إجمالي المنتجات الجديدة" value={formatCurrency(totals?.newItemsTotal || totals?.grandTotal)} />
              <Summary luxury={luxury} label={`رصيد الاستبدال من الفاتورة ${totals?.exchangeInvoiceNumber || ""}`.trim()} value={`- ${formatCurrency(totals?.exchangeCredit)}`} />
              <Summary luxury={luxury} label="المبلغ المدفوع الآن" value={formatCurrency(totals?.amountPaidNow)} />
              {Number(totals?.remainingCustomerCredit || 0) > 0 ? <Summary luxury={luxury} label="الرصيد المتبقي للعميل" value={formatCurrency(totals?.remainingCustomerCredit)} /> : null}
            </>
          ) : null}
