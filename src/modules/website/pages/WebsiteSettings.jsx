    ...
    <div className="space-y-5 pb-24" dir={isArabic ? "rtl" : "ltr"}>
      <header className="rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-xl shadow-[var(--shadow)]">
        ...
        <div className="mt-5 inline-flex rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--card)] p-1">
          {[
            ...
            <button key={key} type="button" onClick={() => setActiveTab(key)} className={`h-[var(--control-height-md)] rounded-[var(--radius-control)] px-4 text-sm font-black transition ${activeTab === key ? "bg-[var(--primary)] text-white shadow" : "text-[var(--muted)] hover:text-[var(--text)]"}`}>
              {label}
            </button>
          ))}
        </div>
            ...
            <div className="border-b border-[var(--border)] p-5">
              <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button type="button" onClick={() => addZone()} className="inline-flex h-[var(--control-height-md)] items-center gap-2 rounded-[var(--radius-control)] bg-[var(--primary)] px-3 text-xs font-black text-white">
                    <Plus className="h-4 w-4" />
                    {copy.addZone}
                  </button>
                  <button type="button" onClick={() => importInputRef.current?.click()} className="inline-flex h-[var(--control-height-md)] items-center gap-2 rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--card)] px-3 text-xs font-black text-[var(--text)]">
                    <FileUp className="h-4 w-4" />
                    {copy.import}
                  </button>
                  <button type="button" onClick={exportZones} className="inline-flex h-[var(--control-height-md)] items-center gap-2 rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--card)] px-3 text-xs font-black text-[var(--text)]">
              ...
              <div className="mt-5 grid gap-3 xl:grid-cols-[1fr_220px_180px]">
                <label className="relative block">
                  ...
                  <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={copy.search} className="h-[var(--control-height-lg)] w-full rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--card)] px-10 text-sm font-semibold text-[var(--text)] outline-none" />
                </label>
                <select value={governorateFilter} onChange={(event) => setGovernorateFilter(event.target.value)} className="h-[var(--control-height-lg)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--card)] px-3 text-sm font-bold text-[var(--text)] outline-none">
              ...
              <div className="mt-4 rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--card)] p-3">
                ...
                <div className="grid gap-2 lg:grid-cols-[210px_1fr_1fr_1fr_110px_110px]">
                  ...
                  <input value={quickZone.governorate} onChange={(event) => setQuickZone((current) => ({ ...current, governorate: event.target.value }))} placeholder={copy.governorate} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-2 text-xs font-bold text-[var(--text)]" />
                  <input disabled={quickMode === "governorate"} value={quickZone.city} onChange={(event) => setQuickZone((current) => ({ ...current, city: event.target.value }))} placeholder={copy.city} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-2 text-xs font-bold text-[var(--text)] disabled:opacity-40" />
                  <input disabled={quickMode !== "area"} value={quickZone.area} onChange={(event) => setQuickZone((current) => ({ ...current, area: event.target.value }))} placeholder={copy.area} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-2 text-xs font-bold text-[var(--text)] disabled:opacity-40" />
                  <input type="number" min="0" value={quickZone.price} onChange={(event) => setQuickZone((current) => ({ ...current, price: event.target.value }))} placeholder={copy.price} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-2 text-xs font-bold text-[var(--text)]" />
                  <button type="button" onClick={addQuickZone} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] bg-[var(--primary)] px-3 text-xs font-black text-white">{copy.addZone}</button>
                </div>
              </div>
              ...
              {selectedCount ? (
                <div className="mt-4 flex flex-wrap items-center gap-2 rounded-xl border border-primary/20 bg-primary/10 p-3">
                  ...
                  <input type="number" min="0" value={bulkPrice} onChange={(event) => setBulkPrice(event.target.value)} placeholder={copy.price} className="h-[var(--control-height-md)] w-28 rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-2 text-xs font-bold text-[var(--text)]" />
                  <button type="button" onClick={() => bulkPrice !== "" && applyBulk({ price: Number(bulkPrice) })} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] bg-[var(--primary)] px-3 text-xs font-black text-white">{copy.bulkPrice}</button>
                  <button type="button" onClick={() => applyBulk({ cod_allowed: true })} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--card)] px-3 text-xs font-black text-[var(--text)]">{copy.bulkCodOn}</button>
                  <button type="button" onClick={() => applyBulk({ cod_allowed: false })} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--card)] px-3 text-xs font-black text-[var(--text)]">{copy.bulkCodOff}</button>
                  <button type="button" onClick={() => applyBulk({ requires_shipping_proof: true })} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--card)] px-3 text-xs font-black text-[var(--text)]">{copy.bulkProofOn}</button>
                  <button type="button" onClick={() => applyBulk({ requires_shipping_proof: false })} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--card)] px-3 text-xs font-black text-[var(--text)]">{copy.bulkProofOff}</button>
                  <input value={bulkEta} onChange={(event) => setBulkEta(event.target.value)} placeholder={copy.eta} className="h-[var(--control-height-md)] w-44 rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-2 text-xs font-bold text-[var(--text)]" />
                  <button type="button" onClick={() => bulkEta && applyBulk({ estimated_delivery_text: bulkEta })} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--card)] px-3 text-xs font-black text-[var(--text)]">{copy.bulkEta}</button>
                  <button type="button" onClick={deleteSelected} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] bg-rose-600 px-3 text-xs font-black text-white">{copy.deleteSelected}</button>
                </div>
              ) : null}
            </div>
                  ...
                  {filteredZones.map((zone) => (
                    <tr key={zone.id} className="border-t border-[var(--border)]">
                      ...
                      <td className="px-3 py-3">
                        <button type="button" onClick={() => { setZones((current) => current.filter((item) => item.id !== zone.id)); setDirty(true); }} className="inline-flex h-[var(--control-height-md)] w-9 items-center justify-center rounded-[var(--radius-control)] border border-rose-400/20 bg-rose-500/10 text-rose-300">
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-[var(--border)] bg-[var(--surface)]/95 px-4 py-3 shadow-2xl shadow-black/20 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3">
          ...
          <button type="button" onClick={saveAll} disabled={loading || saving} className="inline-flex h-[var(--control-height-lg)] items-center justify-center gap-2 rounded-[var(--radius-control)] bg-[var(--primary)] px-5 text-sm font-black text-white shadow-lg disabled:cursor-not-allowed disabled:opacity-60">
...
function EditableCell({ value, onChange, type = "text", compact = false, wide = false }) {
  return (
    <td className="border-t border-[var(--border)] px-3 py-3">
      <input type={type} value={value ?? ""} onChange={(event) => onChange(event.target.value)} className={`h-[var(--control-height-md)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--card)] px-2 text-sm font-semibold text-[var(--text)] outline-none ${compact ? "w-24" : wide ? "w-48" : "w-36"}`} />
    </td>
  );
}
