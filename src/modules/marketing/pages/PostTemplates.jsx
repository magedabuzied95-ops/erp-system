...
export default function PostTemplates() {
  ...
  return (
    <div className="min-h-full bg-[var(--card)] text-[var(--text)]">
      <div className="mx-auto flex w-full flex-col gap-6 px-4 py-6 md:px-6 lg:px-8">
        ...
        <section className="rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-[var(--shadow-card)]">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {TEMPLATE_LIBRARY.map((template) => {
              ...
              return (
                <article key={template.id} className={`relative overflow-hidden rounded-[28px] border border-[var(--border)] bg-gradient-to-br ${template.accent} p-4 shadow-[var(--shadow-card)]`}>
                  <div className="absolute inset-0 bg-[var(--surface)]" />
                  <div className="relative flex h-full min-h-[220px] flex-col">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex h-11 w-11 items-center justify-center rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface)] text-[var(--text)]">
                        <Icon className="h-5 w-5" />
                      </div>
                      <span className="rounded-full border border-[var(--border)] bg-[var(--surface)] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--text)]">
                        {template.type}
                      </span>
                    </div>
                    <div className="mt-5">
                      <div className="text-xl font-black tracking-tight text-[var(--text)]">{template.name}</div>
        ...
        <section className="rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-[var(--shadow-card)]">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {loading ? (
              <div className="rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface)] p-6 text-sm text-[var(--muted)]">{t("marketing.templates.loading")}</div>
            ) : templates.length === 0 ? (
              <div className="rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface)] p-6 text-sm text-[var(--muted)]">{t("marketing.templates.empty")}</div>
            ) : (
              templates.map((template) => (
                <div key={String(template.id)} className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="text-lg font-black text-[var(--text)]">{template.name}</div>
                      <div className="text-sm text-[var(--muted)]">{template.channel}</div>
                    </div>
                  ...
                  <div className="mt-4 flex gap-2">
                    {canUpdate ? <button onClick={() => openEdit(template)} className="rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-xs font-semibold text-[var(--text)]">{t("marketing.common.edit")}</button> : null}
                    {canDelete ? (
