...
const Badge = ({ children, tone = "slate" }) => {
  const tones = {
    ...
    draft: "border-[var(--border)] bg-[var(--surface)] text-[var(--text)]",
  };
        ...
        <section className="rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--card)] p-5 shadow-[var(--shadow-card)]">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={load} className="inline-flex items-center gap-2 rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-4 py-3 text-sm font-semibold text-[var(--text)] hover:bg-[var(--surface)]">
            ...
            <table className="m1-table m1-table--compact min-w-full">
              <thead>
                <tr className="text-start text-[11px] uppercase tracking-[0.18em] text-[var(--muted)]">
                  <th className="border-b border-[var(--border)] px-3 py-3 font-semibold">{t("marketing.campaigns.headers.name")}</th>
                  <th className="border-b border-[var(--border)] px-3 py-3 font-semibold">{t("marketing.campaigns.headers.status")}</th>
                  <th className="border-b border-[var(--border)] px-3 py-3 font-semibold">{t("marketing.campaigns.headers.budget")}</th>
                  <th className="border-b border-[var(--border)] px-3 py-3 font-semibold">{t("marketing.campaigns.headers.dates")}</th>
                  <th className="border-b border-[var(--border)] px-3 py-3 font-semibold text-end">{t("marketing.campaigns.headers.actions")}</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  <tr><td colSpan={5} className="px-3 py-10 text-center text-sm text-[var(--muted)]">{t("marketing.campaigns.loading")}</td></tr>
                ) : campaigns.length === 0 ? (
                  <tr><td colSpan={5} className="px-3 py-10 text-center text-sm text-[var(--muted)]">{t("marketing.campaigns.empty")}</td></tr>
                ) : (
                  campaigns.map((campaign) => (
                    <tr key={String(campaign.id)} className="align-top">
                      <td className="border-b border-[var(--border)] px-3 py-4">
                        <div className="font-semibold text-[var(--text)]">{campaign.name}</div>
                        <div className="mt-1 text-sm text-[var(--muted)]">{campaign.description || "-"}</div>
                      </td>
                      <td className="border-b border-[var(--border)] px-3 py-4"><Badge tone={campaign.status}>{campaign.status}</Badge></td>
                      <td className="border-b border-[var(--border)] px-3 py-4 text-sm text-[var(--muted)]">{Number(campaign.budget || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                      ...
                      <td className="border-b border-[var(--border)] px-3 py-4">
                        <div className="flex justify-end gap-2">
                          {canUpdate ? (
                            <button onClick={() => openEdit(campaign)} className="rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-xs font-semibold text-[var(--text)]">
