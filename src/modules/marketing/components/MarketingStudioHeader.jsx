...
const tabs = [
  ...
  { to: "/marketing/settings", label: "ربط Meta", icon: Settings2 },
];

const tabClassName = ({ isActive }) =>
  [
    "inline-flex h-9 shrink-0 items-center gap-2 rounded-full border px-4 text-sm font-semibold transition duration-200",
    isActive
...
export default function MarketingStudioHeader({ eyebrow = "Marketing Studio", title = "Marketing Suite", description = "Navigate the marketing workspace from one shared header." }) {
  const { t } = useTranslation();

  return (
    <section className="rounded-3xl border border-white/10 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-4 shadow-2xl shadow-black/25 sm:p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-3xl">
          ...
          <h1 className="mt-2 text-2xl font-semibold tracking-tight text-white sm:mt-3 sm:text-3xl xl:text-4xl">
            {t("marketing.studio.title", title)}
          </h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300 sm:mt-3 xl:text-base">
            {t("marketing.studio.description", description)}
          </p>
        </div>
      </div>
