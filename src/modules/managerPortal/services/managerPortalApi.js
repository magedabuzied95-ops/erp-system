...
export const managerPortalApi = {
  ...
  chatLinkPreview: (token, url) => api.get(`${tokenPath(token)}/chat/link-preview`, { params: { url } }),
  starChatMessage: (token, messageId) => api.post(`${tokenPath(token)}/chat/messages/${encodeURIComponent(messageId)}/star`, {}),
  starredChatMessages: (token) => api.get(`${tokenPath(token)}/chat/starred`),
  updateChatPrefs: (token, threadId, prefs = {}) => api.patch(`${tokenPath(token)}/chat/${encodeURIComponent(threadId)}/prefs`, prefs),
  ...
  taskTemplates: (token, options = {}) => api.get(`${tokenPath(token)}/task-templates`, options),
  createTaskTemplate: (token, payload) => api.post(`${tokenPath(token)}/task-templates`, payload),
  updateTaskTemplate: (token, id, payload) => api.put(`${tokenPath(token)}/task-templates/${encodeURIComponent(id)}`, payload),
  setTaskTemplateActive: (token, id, isActive) => api.patch(`${tokenPath(token)}/task-templates/${encodeURIComponent(id)}/active`, { is_active: isActive }),
  deleteTaskTemplate: (token, id) => api.delete(`${tokenPath(token)}/task-templates/${encodeURIComponent(id)}`),
