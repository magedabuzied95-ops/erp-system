...
const STAFF_TASK_COPY = {
  en: {
    ...
    taskTitle: "Task title", autoAssignEmployee: "Auto assign employee", anyBranch: "Every branch — one task each", photoProof: "Photo proof", qrVerification: "QR verification", gpsValidation: "GPS validation",
    ...
    <div className="grid gap-3 border-b border-[var(--border)] px-4 py-4 text-start last:border-b-0 lg:grid-cols-[1fr_auto]">
      ...
      <div className="flex flex-wrap items-center justify-end gap-2 rtl:justify-start">
        <button
          type="button"
          onClick={() => onEdit(task)}
          className="inline-flex h-[var(--control-height-md)] items-center gap-2 rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--card)] px-3 text-sm font-bold text-[var(--text)] hover:bg-[var(--surface-soft)]"
        >
        ...
        {canDelete ? (
          <button
            ...
            className="inline-flex h-[var(--control-height-md)] items-center gap-2 rounded-[var(--radius-control)] border border-red-300/40 bg-red-500/10 px-3 text-sm font-bold text-red-700"
          >
        ...
        {task.status === "in_progress" ? (
          <button
            ...
            className="inline-flex h-[var(--control-height-md)] items-center gap-2 rounded-[var(--radius-control)] bg-[var(--primary)] px-3 text-sm font-black text-white"
          >
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className="inline-flex h-[var(--control-height-lg)] items-center gap-2 rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--card)] px-4 text-sm font-black text-[var(--text)] hover:bg-[var(--surface-soft)]"
          >
            <Plus className="h-4 w-4" />
            {tr("addWeeklyTask")}
          </button>
          <button
            ...
            className="inline-flex h-[var(--control-height-lg)] items-center gap-2 rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--card)] px-4 text-sm font-black text-[var(--text)] hover:bg-[var(--surface-soft)] disabled:opacity-60"
          >
          ...
          <button
            ...
            className="inline-flex h-[var(--control-height-lg)] items-center gap-2 rounded-[var(--radius-control)] bg-[var(--primary)] px-4 text-sm font-black text-white disabled:opacity-60"
          >
      ...
      {panelOpen ? (
        <section className="rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--card)] p-4 shadow-sm">
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            <button type="button" onClick={() => setPanelOpen(false)} className="rounded-[var(--radius-control)] border border-[var(--border)] p-2 text-[var(--muted)]">
              <X className="h-4 w-4" />
            </button>
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            <input value={form.title} onChange={(e) => setForm((v) => ({ ...v, title: e.target.value }))} placeholder={tr("taskTitle")} className="h-[var(--control-height-lg)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-3 text-start text-sm font-semibold text-[var(--text)] outline-none" />
            <select value={form.branch_id} onChange={(e) => setForm((v) => ({ ...v, branch_id: e.target.value }))} className="h-[var(--control-height-lg)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-3 text-sm font-semibold text-[var(--text)] outline-none">
          ...
          <div className="mt-4 flex justify-end">
            <button type="button" disabled={busy === "save-task" || !form.title.trim()} onClick={saveTask} className="inline-flex h-[var(--control-height-lg)] items-center gap-2 rounded-[var(--radius-control)] bg-[var(--primary)] px-4 text-sm font-black text-white disabled:opacity-60">
        ...
        <section className="min-w-0 rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--card)]">
          ...
          <div className="flex flex-col gap-3 border-b border-[var(--border)] p-4">
            ...
            <div className="grid gap-2 md:grid-cols-5">
              <select value={status} onChange={(event) => setStatus(event.target.value)} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-3 text-sm font-semibold text-[var(--text)] outline-none">
              ...
              <select value={employeeFilter} onChange={(event) => setEmployeeFilter(event.target.value)} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-3 text-sm font-semibold text-[var(--text)] outline-none">
                <option value="">{tr("allEmployees")}</option>
                {employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.full_name || employee.employee_code}</option>)}
              </select>
              <select value={branchFilter} onChange={(event) => setBranchFilter(event.target.value)} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-3 text-sm font-semibold text-[var(--text)] outline-none">
                <option value="">{tr("allBranches")}</option>
                {branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name || branch.branch_name}</option>)}
              </select>
              <select value={priorityFilter} onChange={(event) => setPriorityFilter(event.target.value)} className="h-[var(--control-height-md)] rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-3 text-sm font-semibold text-[var(--text)] outline-none">
