    ...
    <div dir="ltr" className="space-y-4 p-4 text-white md:p-6">
      <section className="rounded-[var(--radius-card)] border border-white/10 bg-white/[0.055] px-5 py-4 shadow-[0_18px_60px_rgba(0,0,0,0.16)] backdrop-blur">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.18em] text-primary"><Activity className="h-4 w-4" />{t("aiStudio.pages.eyebrow")}</div>
            <h1 className="m1-page-title mt-1">{t("aiStudio.pages.executions.title")}</h1>
            <p className="mt-1 text-sm text-slate-400">{t("aiStudio.pages.executions.subtitle")}</p>
          </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <section className="overflow-hidden rounded-[var(--radius-card)] border border-white/10 bg-white/[0.03]">
          {loading ? (
            ...
            <div className="p-8 text-center text-sm text-slate-500">{t("aiStudio.pages.executions.empty")}</div>
          ) : (
              ...
              {runs.map((r) => (
                <button key={r.id} type="button" onClick={() => openRun(r.id)} className={`flex w-full items-center justify-between gap-3 border-b border-white/5 px-4 py-3 text-left hover:bg-white/[0.05] ${selected === r.id ? "bg-white/[0.06]" : ""}`}>
                  <span className="min-w-0">
                    <span className="block truncate text-sm font-black text-white">{r.workflow_name || t("aiStudio.pages.executions.workflowNumber", { id: r.workflow_id })}</span>
                  ...
                  <span className={`shrink-0 text-[11px] font-black ${statusTone(r.status)}`}>{statusLabel(r.status)}{r.pending_node_id ? ` @${r.pending_node_id}` : ""}</span>
                </button>
              ))}
            </div>
          )}
        ...
        <section className="overflow-hidden rounded-[var(--radius-card)] border border-white/10 bg-white/[0.03]">
          {!selected ? (
            <div className="p-8 text-center text-sm text-slate-500">{t("aiStudio.pages.executions.selectRun")}</div>
          ) : !detail ? (
            <div className="flex items-center gap-2 p-6 text-sm text-slate-400"><Loader2 className="h-4 w-4 animate-spin" />{t("aiStudio.pages.executions.loadingRun")}</div>
          ) : (
            <div className="p-4">
              <div className="mb-3 flex items-center justify-between">
                <div className="text-sm font-black text-white">{t("aiStudio.pages.executions.run", { id: detail.run?.id })} · <span className={statusTone(detail.run?.status)}>{statusLabel(detail.run?.status)}</span></div>
              ...
              <div className="mb-3 flex flex-wrap items-center gap-2 text-[11px] text-slate-400">
                <span>{t("aiStudio.pages.executions.trigger")} <span className={`font-black ${detail.run?.trigger && detail.run.trigger !== "manual" ? "text-violet-200" : "text-slate-200"}`}>{triggerLabel(detail.run?.trigger)}</span></span>
                <span className="text-slate-600">·</span>
                <span>{detail.run?.trigger && detail.run.trigger !== "manual" ? t("aiStudio.pages.executions.automatic") : t("aiStudio.pages.executions.manual")}</span>
                {detail.run?.event_id ? <><span className="text-slate-600">·</span><span>{t("aiStudio.pages.executions.event")} <span className="font-mono text-slate-300">{detail.run.event_id}</span></span></> : null}
              </div>
                ...
                {(detail.steps || []).map((s) => (
                  <li key={s.id} className="rounded-xl border border-white/10 bg-slate-950/40 p-3">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className={`text-[11px] font-black ${statusTone(s.status)}`}>{statusLabel(s.status)}{typeof s.duration_ms === "number" ? ` · ${s.duration_ms}ms` : ""}</span>
                    </div>
