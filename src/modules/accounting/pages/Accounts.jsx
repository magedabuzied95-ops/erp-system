...
function Accounts() {
  ...
  const accountTypeOptions = [
    { value: "", label: tt("accounting.ledgers.filters.allAccounts") },
    { value: "asset", label: tt("accounting.accountGroups.assets") },
    { value: "liability", label: tt("accounting.accountGroups.liabilities") },
    { value: "equity", label: tt("accounting.accountTypes.equity") },
    { value: "revenue", label: tt("accounting.accountGroups.revenues") },
    { value: "expense", label: tt("accounting.accountGroups.expenses") },
  ];
  ...
  const loadAccounts = async () => {
    ...
    try {
      ...
      setAccountsError(requestError?.message || tt("accounting.accounts.errors.loadTree"));
  ...
  const loadReport = async (params = filters) => {
    ...
    try {
      ...
      setError(requestError?.message || tt("accounting.ledgers.errors.load"));
      ...
      {view === "chart" ? (
        <>
          {accountsError ? (
            <StateBanner
              ...
              title={tt("accounting.accounts.errors.load")}
              text={accountsError}
              action={
                <button type="button" onClick={loadAccounts} className="inline-flex items-center gap-2 rounded-[var(--radius-control)] border border-white/10 bg-white/5 px-4 py-2 text-sm font-black text-white transition hover:bg-white/10">
                  ...
                  {tt("accounting.common.actions.retry")}
                </button>
              }
            />
          ) : null}
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <FinanceMetricCard label={tt("accounting.accounts.metrics.total")} value={accountsData.summary.total || 0} tone="cyan" icon={<BookCopy className="h-5 w-5" />} />
            <FinanceMetricCard label={tt("accounting.accounts.metrics.active")} value={accountsData.summary.active || 0} tone="emerald" icon={<Wallet className="h-5 w-5" />} />
            <FinanceMetricCard label={tt("accounting.accounts.metrics.assets")} value={accountsData.summary.by_type?.asset || 0} tone="amber" icon={<ArrowDownLeft className="h-5 w-5" />} />
            <FinanceMetricCard label={tt("accounting.accounts.metrics.expenses")} value={accountsData.summary.by_type?.expense || 0} tone="rose" icon={<ArrowUpRight className="h-5 w-5" />} />
          </div>
          ...
          <div className="rounded-3xl border border-white/10 bg-zinc-950/90 p-5 shadow-2xl shadow-black/10">
              ...
              <div className="flex gap-2">
                <label className="flex min-w-[260px] items-center gap-3 rounded-[var(--radius-card)] border border-white/10 bg-white/5 px-4 py-3 text-zinc-300">
                  ...
                  <input
                    ...
                    placeholder={tt("accounting.accounts.searchPlaceholder")}
            ...
            {accountsLoading ? (
              <div className="mt-5 rounded-[var(--radius-card)] border border-white/10 bg-white/5 p-8 text-sm text-zinc-400">{tt("accounting.accounts.loading")}</div>
                ...
                <table className="m1-table m1-table--compact min-w-[860px] w-full text-right text-sm" dir="rtl">
                  <thead className="bg-white/5 text-[11px] uppercase tracking-[0.18em] text-zinc-500">
                    <tr>
                      ...
                      <Th align="right">{tt("accounting.common.labels.accountName")}</Th>
                      <Th align="right">{tt("accounting.common.labels.type")}</Th>
                      <Th align="right">{tt("accounting.accounts.parent")}</Th>
                      <Th align="right">{tt("orders.table.status")}</Th>
                      <Th align="right">{tt("accounting.common.labels.created")}</Th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={account.id} className="transition hover:bg-white/[0.03]">
                        ...
                        <Td align="right">
                          <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-black ${account.is_active ? "bg-emerald-500/15 text-emerald-300" : "bg-zinc-500/15 text-zinc-300"}`}>
                            {account.is_active ? tt("orders.details.active") : tt("accounting.accounts.suspended")}
                          </span>
                        </Td>
        ...
        <>
          <form onSubmit={applyFilters} className="grid gap-3 rounded-3xl border border-white/10 bg-zinc-950/90 p-4 shadow-2xl shadow-black/10 md:grid-cols-5">
            ...
            <div className="flex items-end gap-2">
              <button type="submit" className="inline-flex w-full items-center justify-center gap-2 rounded-[var(--radius-control)] bg-primary px-4 py-2 text-sm font-black text-black transition hover:bg-primary">
                ...
                {tt("accounting.common.actions.apply")}
              </button>
            </div>
          </form>
          ...
          {error ? (
            <StateBanner
              icon={<AlertTriangle className="h-5 w-5" />}
              title={tt("accounting.ledgers.errors.load")}
              text={error}
              action={
          ...
          <div className="overflow-hidden rounded-3xl border border-white/10 bg-zinc-950/90 shadow-2xl shadow-black/10">
            <div className="flex flex-col gap-1 border-b border-white/10 p-5 md:flex-row md:items-end md:justify-between">
              <div>
                <h3 className="m1-section-title text-white">{tt("accounting.tabs.ledgers")}</h3>
...
function translateSourceType(value) {
  ...
  if (normalized === "ledger") return tt("accounting.ledgers.ledger");
  ...
  if (normalized === "cash") return tt("accounting.accountTypes.cash");
