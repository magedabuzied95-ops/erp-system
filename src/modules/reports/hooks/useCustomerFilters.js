...
/**
 * R6 filter state. Extends the shared period/comparison hook with the breakdown
 * dimension, the channel and branch filters, and the customer table's sort/search/page.
 *
 * URL-backed like every other Reporting Center page, so a shared link reproduces the
 * view exactly.
 */

export const CUSTOMER_DIMENSIONS = ["segment", "tier", "channel", "branch"];
export const DEFAULT_DIMENSION = "segment";

export const CUSTOMER_SORTS = ["net_sales", "orders", "units", "average_order", "last_order", "first_order"];

/** Pure, so the allowlist that protects a user-editable URL can be tested on its own. */
export const normaliseCustomerFilters = (searchParams) => {
  const get = (key) => searchParams.get(key) || "";
  const dimension = searchParams.get("dimension");
  const sort = searchParams.get("sort");

  return {
    dimension: CUSTOMER_DIMENSIONS.includes(dimension) ? dimension : DEFAULT_DIMENSION,
    sort: CUSTOMER_SORTS.includes(sort) ? sort : "net_sales",
...
export default function useCustomerFilters() {
  ...
  const { dimension, sort, sortDir, channel, branchId, search, page } = normaliseCustomerFilters(searchParams);
  ...
  const clearFilters = useCallback(() => patch({ channel: null, branchId: null, q: null }), [patch]);
  ...
  return {
    ...base,
    dimension, sort, sortDir, channel, branchId, search, page,
    activeFilterCount,
    analyticalParams, breakdownParams, listParams,
