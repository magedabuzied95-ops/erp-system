...
const AIInboxAnalysisPanel = memo(function AIInboxAnalysisPanel({ analysis, copilot, loading, cacheHit, onTrack, flags }) {
  ...
  return (
    <section className="mb-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm" aria-label="AI conversation intelligence">
      {intelligence ? (
        <div className="flex flex-wrap gap-1.5 text-[10px] font-black text-slate-700">
          ...
          <span className="rounded-full bg-slate-100 px-2 py-1">Lead {intelligence.leadScore}</span>
          ...
          {intelligence.objections.map((objection) => <span key={objection} className="rounded-full bg-slate-100 px-2 py-1">Objection: {objection}</span>)}
        </div>
      ) : null}
      {flags.COPILOT_ENABLED && copilot ? (
        <div className="mt-3 border-t border-slate-200 pt-3">
          <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.14em] text-slate-500"><Sparkles className="h-3.5 w-3.5" />Copilot</div>
      ...
      {debug ? (
        <details className="mt-3 border-t border-slate-200 pt-2 text-[10px] text-slate-500">
          <summary className="cursor-pointer font-black">AI DEBUG</summary>
