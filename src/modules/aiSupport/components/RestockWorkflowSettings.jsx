...
// in one panel: global automation (env, read-only here), tenant automation, the
// "Restock Customer Recovery" workflow, its restock.recover grant, and the
// messaging mode. The effective outcome is derived from all of them so staff
// never have to reason about which switch is the one still off.
...
export default function RestockWorkflowSettings({ open = false }) {
  ...
  const load = useCallback(async () => {
    ...
    setMsg("");
    try {
      const [recovery, mode, notifications] = await Promise.all([
        getRestockRecovery(),
        getRestockMessagingMode(),
  ...
  return (
    <div className="mt-2 space-y-2 rounded-2xl border border-[var(--border)] bg-[var(--surface-soft)] p-3">
      <div className="flex items-center justify-between gap-2">
        <div className="text-[11px] font-black uppercase tracking-[0.12em] text-[var(--muted)]">{t("aiSupport.inbox.customer360.restockSettingsTitle")}</div>
        <button type="button" onClick={() => void load()} disabled={loading} className="inline-flex items-center gap-1 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-2 py-1 text-[10px] font-black text-[var(--text-secondary)] disabled:opacity-50">
      ...
      <div className="flex items-center justify-between gap-2 text-[10px] text-[var(--muted)]">
        <span>{t("aiSupport.inbox.customer360.restockCooldownNote")}</span>
        <button type="button" onClick={() => navigate("/ai-studio/restock-recovery")} className="inline-flex shrink-0 items-center gap-1 font-black text-[var(--primary)]">
          {t("aiSupport.inbox.customer360.restockOpenStudio", { count: state.pendingApproval })} <ExternalLink className="h-3 w-3" />
        </button>
      </div>

      {msg ? <div className="rounded-xl border border-rose-300/40 bg-rose-500/10 px-3 py-1.5 text-xs font-semibold text-rose-600">{msg}</div> : null}
    </div>
  );
}
