...
const couponErrorText = (reason = "") => {
  ...
  return sfText(key, String(reason || "").trim() || sfText("storefront.checkout.couponErrors.invalid", "كود الكوبون غير صالح"));
};
...
const normalizeStorefrontPaymentSettings = (settings = {}) => {
  ...
  return {
    ...
    shippingConfirmation: {
      ...
      label: text(settings["storefront.payment_methods.shipping_confirmation_label"], "تأكيد الشحن"),
    },
  };
};
...
const mainHomeCategoryCards = [
  {
    ...
    titleAr: "رجالي",
    titleEn: "Men",
    subtitleAr: "أحدث Nike وAdidas وJordan",
  ...
  },
  {
    ...
    titleEn: "Last Sizes",
    subtitleAr: "أزواج محدودة قبل نفادها",
...
function HomePage(props) {
  ...
  return (
    <div className="sf-page pb-[calc(var(--mobile-bottom-nav-height,76px)+env(safe-area-inset-bottom)+1.5rem)] md:pb-0">
      ...
      <HomeProductSection title={normalizeLanguage(lang) === "ar" ? "وصل حديثًا" : t("storefront.nav.new", "جديد")} subtitle={normalizeLanguage(lang) === "ar" ? "أضيفت حديثًا إلى المخزون" : t("storefront.home.newSubtitle", "وصل حديثًا للمخزون")} viewAllTo="/shop/products?sort=newest" loading={loading || storefrontHome.loading} products={homeSections.newArrivals} railType="new" tone="new" {...props} />
      <HomeProductSection title={normalizeLanguage(lang) === "ar" ? "العروض" : t("storefront.nav.sale", "العروض")} subtitle={normalizeLanguage(lang) === "ar" ? "خصومات مختارة لفترة محدودة" : t("storefront.home.saleSubtitle", "خصومات مختارة لفترة محدودة")} viewAllTo="/shop/products?sale=true" loading={saleLoading && !homeSections.sale.length} products={homeSections.sale} railType="sale" tone="sale" {...props} />
      <HomeProductSection title={normalizeLanguage(lang) === "ar" ? "آخر المقاسات" : t("storefront.home.lastSizes", "Last Sizes")} subtitle={normalizeLanguage(lang) === "ar" ? "أزواج محدودة قبل نفادها" : t("storefront.home.lastSizesSubtitle", "Limited pairs before they disappear")} viewAllTo="/shop/products?lastSizes=true" loading={loading || storefrontHome.loading} products={homeSections.lastSizes} railType="last-size" tone="last" {...props} />
      <HomeProductSection title={normalizeLanguage(lang) === "ar" ? "الأكثر رواجًا" : "Trending"} subtitle={normalizeLanguage(lang) === "ar" ? "اختيارات شائعة مع أحدث المنتجات كبديل." : "Popular picks, with newest products as fallback."} viewAllTo="/shop/products?sort=trending" loading={loading || storefrontHome.loading} products={homeSections.trending} railType="trending" tone="trending" {...props} />
      <Reviews />
      <HomeBrandsSection />
...
function MiniRailEmpty() {
  ...
  return (
    <div className="rounded-[1.5rem] border border-[#8b5cf6]/18 bg-[linear-gradient(180deg,rgba(18,18,28,0.96),rgba(7,10,20,0.94))] p-6 text-center text-stone-50 shadow-[0_18px_45px_rgba(0,0,0,0.45),inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-xl">
      ...
      <h3 className="mt-3 text-lg font-black text-stone-50">{t("storefront.products.emptyRailTitle", "لسه بنجهز منتجات مميزة هنا")}</h3>
      <p className="mt-1 text-sm font-bold text-stone-400">{t("storefront.products.comingSoon", "وصل قريبًا")}</p>
    </div>
  );
}
  ...
  return (
    <section className="scroll-mt-28">
      <div className="mb-2.5 flex items-end justify-between gap-2 md:mb-3 md:gap-3">
        <SectionIntro eyebrow={t("storefront.filters.gender", "الجنس")} title={t("storefront.products.chooseWearer", "اختار مين هيلبس")} subtitle={t("storefront.products.chooseWearerSubtitle", "بعد الاختيار هننقلك لنوع المنتج")} compact />
      </div>
        ...
        {options.map((option) => {
          ...
          return (
            ...
            >
              ...
              <span className="mt-0.5 block text-[9.5px] font-bold leading-3 text-stone-500 dark:text-stone-400 md:mt-1 md:text-xs md:leading-5">{t("storefront.products.chooseAndViewTypes", "اختار وشوف الأنواع")}</span>
            </button>
          );
        })}
      </div>
        ...
        {loading ? <ProductTypeSkeleton /> : options.map((option) => {
          ...
          return (
            ...
            >
              ...
              {Number.isFinite(Number(count)) ? <span className="mt-0.5 block text-[9.5px] font-bold leading-3 text-stone-500 dark:text-stone-400 md:mt-1 md:text-[11px] md:leading-4">{t("storefront.products.productCount", "{{count}} منتج", { count })}</span> : null}
            </button>
          );
        })}
      </div>
      {!loading && !options.length ? <EmptyState title={t("storefront.products.noTypesForCategory", "مفيش أنواع متاحة للقسم ده")} text={t("storefront.products.goBackChooseAnother", "ارجع واختار نوع تاني")} /> : null}
    </div>
  );
}
  ...
  return (
    <div className={`mb-2.5 rounded-[0.9rem] border border-stone-200 bg-white p-2 shadow-[0_10px_24px_rgba(39,20,75,0.05)] dark:border-white/10 dark:bg-[#0b1020] md:mb-4 md:rounded-[1.35rem] md:p-3 ${disabled ? "opacity-55" : ""}`}>
      <div className="mb-1.5 flex items-center justify-between gap-2 md:mb-2 md:gap-3">
        <div>
          <p className="text-[8.5px] font-black uppercase tracking-[0.14em] text-[#7c3aed] md:text-[10px] md:tracking-[0.18em]">{t("storefront.filters.sizeFilter", "فلتر المقاس")}</p>
          <h3 className="text-xs font-black md:text-sm">{t("storefront.filters.availableSize", "فلتر المقاس المتاح")}</h3>
        </div>
        {selectedSize ? (
      ...
      <div className="sf-scroll flex gap-1.5 overflow-x-auto pb-0.5 md:gap-2 md:pb-1">
        ...
        {!sizes.length ? <span className="rounded-full border border-dashed border-stone-200 px-2.5 py-1.5 text-[10.5px] font-bold text-stone-400 dark:border-white/10 md:px-4 md:py-2 md:text-xs">{t("storefront.filters.sizesAppearAfterType", "المقاسات هتظهر بعد اختيار نوع المنتج")}</span> : null}
      </div>
    </div>
  );
}
    ...
    <div className="mb-5 hidden md:block">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.22em] text-[#7c3aed]">{t("storefront.filters.curatedFilters", "فلاتر مختارة")}</p>
            <h2 className="text-sm font-black text-stone-950 dark:text-white">{t("storefront.filters.quickPremium", "فلترة سريعة بتجربة Premium")}</h2>
          </div>
        </div>
        {activeFilterCount ? (
          <Link
          ...
          >
            {t("storefront.filters.clearFilters", "مسح الفلاتر")}
          </Link>
        ) : null}
      </div>
...
function MobileFilterTrigger({ activeFilterCount = 0, onOpen }) {
  ...
  return (
    ...
    >
      <SlidersHorizontal className="h-4 w-4" />
      <span>{t("storefront.filters.filters", "الفلاتر")}</span>
...
function MobileFilterDrawer({ open, sections, lang, draftFilters, setDraftFilters, onClose, onApply, onReset }) {
  ...
  return (
    <div className="sf-mobile-filter-drawer fixed inset-0 z-50 md:hidden" role="dialog" aria-modal="true">
      <button type="button" className="absolute inset-0 bg-stone-950/55 backdrop-blur-sm" onClick={onClose} aria-label={t("storefront.filters.closeFilters", "إغلاق الفلاتر")} />
      <div className="absolute inset-x-0 bottom-0 max-h-[82dvh] overflow-hidden rounded-t-[1.5rem] border border-white/10 bg-[linear-gradient(180deg,#101426_0%,#070b16_100%)] text-white shadow-[0_-24px_70px_rgba(0,0,0,0.42)]">
        ...
        <div className="flex items-center justify-between gap-3 border-b border-white/10 px-3.5 py-3">
          <div>
            <p className="text-[9px] font-black uppercase tracking-[0.18em] text-[#d8b4fe]">{t("storefront.filters.premiumFilters", "فلاتر Premium")}</p>
            <h2 className="text-base font-black">{t("storefront.filters.chooseWhatFits", "اختار اللي يناسبك")}</h2>
          </div>
        ...
        <div className="absolute inset-x-0 bottom-0 flex gap-2 border-t border-white/10 bg-[#070b16]/92 px-3 py-2.5 pb-[calc(env(safe-area-inset-bottom)+0.625rem)] backdrop-blur-xl">
          <button type="button" onClick={onApply} className="flex-1 rounded-xl bg-gradient-to-l from-[#7c3aed] to-[#111827] px-4 py-2.5 text-sm font-black text-white shadow-[0_14px_34px_rgba(124,58,237,0.32)] active:scale-[0.98]">
            {t("storefront.filters.applyFilters", "تطبيق الفلاتر")}
          </button>
    ...
    <article ref={cardRef} style={eagerImage ? undefined : { contentVisibility: "auto", containIntrinsicSize: "240px 400px" }} onClick={openDetails} onMouseEnter={requestDetailPrefetch} onTouchStart={requestDetailPrefetch} className={`sf-product-card group/product relative flex h-full min-h-0 transform-gpu cursor-pointer flex-col overflow-hidden rounded-[1.2rem] border border-white/70 bg-[linear-gradient(145deg,rgba(255,255,255,0.98),rgba(250,248,244,0.94)_48%,rgba(245,241,234,0.84))] shadow-[0_10px_26px_rgba(39,20,75,0.07),inset_0_1px_0_rgba(255,255,255,0.8)] ring-1 ring-stone-200/55 transition-[transform,box-shadow,border-color,background-color] duration-200 ease-out hover:-translate-y-1 hover:border-[#a78bfa]/38 hover:ring-[#7c3aed]/22 hover:shadow-[0_18px_44px_rgba(39,20,75,0.12),0_0_0_1px_rgba(124,58,237,0.08)_inset] md:rounded-[1.55rem] dark:border-white/[0.08] dark:bg-[linear-gradient(145deg,rgba(17,24,39,0.95),rgba(11,16,32,0.93)_52%,rgba(8,13,25,0.98))] dark:ring-white/[0.05] dark:shadow-[0_14px_34px_rgba(0,0,0,0.24),inset_0_1px_0_rgba(255,255,255,0.05)] dark:hover:border-[#a78bfa]/24 dark:hover:shadow-[0_20px_54px_rgba(0,0,0,0.32),0_0_24px_rgba(124,58,237,0.10)] ${featured ? "md:shadow-[0_18px_52px_rgba(109,40,217,0.12)]" : ""}`}>
      ...
      <div className={`relative overflow-visible bg-[radial-gradient(circle_at_50%_42%,rgba(167,139,250,0.12),transparent_28%),linear-gradient(180deg,#fbfaf7_0%,#eee7dc_100%)] md:p-3 dark:bg-[radial-gradient(circle_at_50%_42%,rgba(167,139,250,0.10),transparent_28%),linear-gradient(180deg,#101426_0%,#0b1020_100%)] ${densityClasses.image}`}>
        ...
        >
          ...
          {product.low_stock ? <span className="absolute bottom-2 right-2 z-20 inline-flex h-5 items-center rounded-full border border-amber-200 bg-amber-50/95 px-2 text-[9px] font-black leading-none text-amber-800 shadow-sm backdrop-blur md:bottom-auto md:left-12 md:right-auto md:top-2.5 md:h-6 md:px-2.5 md:text-[10px] dark:border-amber-300/20 dark:bg-amber-400/10 dark:text-amber-100">{t("storefront.products.onlyLeft", "باقي {{count}} فقط", { count: product.total_stock })}</span> : null}
      </div>
        ...
        <button
          ...
          aria-label={canQuickAdd ? t("storefront.cart.addToCart", "إضافة للسلة") : t("storefront.products.unavailable", "غير متاح")}
        >
          <ShoppingCart className="h-[18px] w-[18px] text-white" />
          {canQuickAdd ? t("storefront.cart.addToCart", "إضافة للسلة") : t("storefront.products.unavailable", "غير متاح")}
        </button>
      </div>
      {variantSheetOpen ? (
    ...
    <div className="sf-product-variant-sheet fixed inset-0 z-[90] pointer-events-auto md:hidden" role="dialog" aria-modal="true">
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#d8b4fe]">{t("storefront.products.chooseSize", "اختار المقاس")}</p>
        ...
        {colorGroups.length > 1 ? (
          <div className="mt-4">
            <div className="mb-2 text-[11px] font-black text-white/50">{t("storefront.products.color", "لون")}</div>
        ...
        <div className="mt-4">
          <div className="mb-2 text-[11px] font-black text-white/50">{t("storefront.products.size", "مقاس")}</div>
        ...
        >
          ...
          {t("storefront.cart.addToCart", "إضافة للسلة")}
        </button>
      </section>
    </div>,
    document.body
...
function CheckoutPage({ cart, clearCart, profile, setProfile, themeMode }) {
  ...
  const checkoutActionLabel = checkoutStep === 1
    ? t("storefront.checkout.actions.continueToAddress", "المتابعة للعنوان")
    : checkoutStep === 2
      ? t("storefront.checkout.actions.continueToPayment", "المتابعة للدفع")
      : normalizedFormPaymentMethod === "cod"
        ? t("storefront.checkout.actions.confirmOrder", "تأكيد الطلب")
        : shippingProofRequired
          ? t("storefront.checkout.actions.uploadProofAndConfirm", "ارفع إثبات التحويل وأكد الطلب")
          : t("storefront.checkout.actions.confirmOrder", "تأكيد الطلب");
                      ...
                      <div className="space-y-2">
                        ...
                        {couponValidation?.valid ? (
                          <div className="rounded-2xl border border-emerald-300/20 bg-emerald-400/10 px-3 py-2 text-xs font-black text-emerald-100">
                            {sfText("storefront.checkout.couponAppliedSummary", "تم تطبيق الكوبون: {{code}} - {{discount}}", {
...
function OrderSuccess({ profile }) {
  ...
  const customerName = order.customer_name || loaded?.customer?.full_name || profile.full_name || t("storefront.customer.dearCustomer", "عميلنا العزيز");
  ...
  const successTitle = isShippingAwaitingVerification ? t("storefront.success.awaitingVerificationTitle", "طلبك في انتظار مراجعة الدفع") : t("storefront.success.confirmedTitle", "تم تأكيد الطلب");
  const successSubtitle = isShippingAwaitingVerification
    ? t("storefront.success.awaitingVerificationSubtitle", "سنراجع إثبات التحويل ثم نبدأ تجهيز الطلب.")
    : t("storefront.success.confirmedSubtitle", "تم تسجيل الطلب وسنبدأ التجهيز.");
  const successStatus = isShippingAwaitingVerification ? t("storefront.status.awaiting_verification", "بانتظار مراجعة الدفع") : statusCopy(order.status || "pending");
  ...
  return (
    <section className="relative mx-auto max-w-6xl px-4 py-6 md:py-10">
      ...
      <div className="mx-auto max-w-3xl text-center">
        ...
        <p className="mt-2 text-lg font-bold text-stone-600">{t("storefront.success.thanks", "شكرا لطلبك")}</p>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_360px]">
        <div className="space-y-4">
          <div className="sf-storefront-card rounded-[2rem] border border-stone-200 bg-white p-5 shadow-[0_18px_50px_rgba(39,20,75,0.07)] md:p-6">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <InfoBox label={t("storefront.orders.expectedDelivery", "التوصيل المتوقع")} value={t("storefront.orders.expectedDeliveryWindow", "من 2 إلى 5 أيام عمل")} />
            </div>
            <div className="sf-info-box mt-4 rounded-2xl bg-stone-50 p-4 text-right">
              <div className="sf-info-label text-xs font-black text-stone-500">{t("storefront.checkout.deliveryAddress", "عنوان التوصيل")}</div>
        ...
        <aside className="sf-storefront-card h-max rounded-[2rem] border border-stone-200 bg-white p-5 shadow-[0_18px_50px_rgba(39,20,75,0.07)] lg:sticky lg:top-24">
          <div className="grid gap-3">
            ...
            {whatsAppHref ? <a href={whatsAppHref} className="rounded-full border border-emerald-200 bg-emerald-50 px-5 py-4 text-center font-black text-emerald-700">{t("storefront.support.whatsapp", "واتساب")}</a> : <button disabled className="rounded-full border border-stone-200 bg-stone-100 px-5 py-4 font-black text-stone-400">{t("storefront.support.whatsappUnavailable", "واتساب غير متاح الآن")}</button>}
          </div>
          <div className="sf-info-box mt-5 rounded-2xl bg-[#f5f3ff] p-4 text-sm font-bold leading-6 text-stone-700">{t("storefront.success.reviewNotice", "احتفظ برقم الطلب للمتابعة.")}</div>
        </aside>
      </div>
      {products.length ? (
        <div className="mt-6">
          <ProductRail title={t("storefront.nav.new", "جديد")} subtitle={t("storefront.success.recommendedProducts", "منتجات مقترحة")} products={products} loading={false} railType="new" wishlist={[]} toggleWishlist={() => undefined} onAddToCart={() => undefined} />
        </div>
      ) : null}
    </section>
  );
...
const supportHref = (orderNumber = "") => {
  const text = orderNumber ? sfText("storefront.support.orderHelpMessage", "مرحبًا، أحتاج مساعدة في الطلب {{orderNumber}}", { orderNumber }) : sfText("storefront.support.generalHelpMessage");
...
function CityAreaField({ governorate, options, value, onChange, manual, onManualChange, required, error, themeMode = "light" }) {
  ...
  return (
    <div className="sf-checkout-field block">
      <span className={`sf-checkout-field-label mb-1.5 block text-sm font-black ${darkMode ? "text-white/82" : "text-slate-800"}`}>المحافظة / المدينة</span>
  ...
  return (
    <div className={`sf-checkout-payment-copy rounded-[1.55rem] border p-4 shadow-[0_22px_54px_rgba(0,0,0,0.26)] ${isVodafone ? "border-red-300/18 bg-[linear-gradient(145deg,rgba(230,0,0,0.16),rgba(255,255,255,0.055))]" : "border-[#a78bfa]/18 bg-[linear-gradient(145deg,rgba(124,58,237,0.18),rgba(255,255,255,0.055))]"}`}>
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-xs font-black text-white/48">{sfText("storefront.checkout.transfer.transferDetailsVia", "بيانات التحويل عبر {{label}}", { label })}</div>
