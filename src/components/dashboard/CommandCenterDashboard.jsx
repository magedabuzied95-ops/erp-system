...
export const CommandCenterDashboard = memo(function CommandCenterDashboard({
  ...
  const kpis = [
    { id: "revenueToday", label: t("dashboard.commandCenter.kpis.revenueToday"), value: metrics.todaySales, displayValue: formatCurrency(metrics.todaySales), trend: overview?.kpis?.todaySales?.growth, icon: Banknote, tone: "emerald" },
    { id: "ordersToday", label: t("dashboard.commandCenter.kpis.ordersToday"), value: metrics.todayOrders, trend: overview?.kpis?.todayOrders?.growth, icon: ReceiptText, tone: "sky" },
    { id: "customersOnline", label: t("dashboard.commandCenter.kpis.customersOnline"), value: metrics.activeCustomers, icon: Users, tone: "violet" },
    { id: "aiConversations", label: t("dashboard.commandCenter.kpis.aiConversations"), value: metrics.activeAiConversations, icon: Bot, tone: "sky" },
    { id: "lowStock", label: t("dashboard.commandCenter.kpis.lowStock"), value: metrics.lowStockProducts, icon: PackageSearch, tone: metrics.lowStockProducts ? "amber" : "emerald" },
    { id: "checkedInStaff", label: t("dashboard.commandCenter.kpis.checkedInStaff"), value: metrics.checkedInStaff, icon: UserCheck, tone: "violet" },
    { id: "abandonedCarts", label: t("dashboard.commandCenter.kpis.abandonedCarts"), value: metrics.abandonedCarts, icon: ShoppingCart, tone: metrics.abandonedCarts ? "rose" : "emerald" },
    { id: "conversionRate", label: t("dashboard.commandCenter.kpis.conversionRate"), value: metrics.conversionRate, displayValue: `${metrics.conversionRate.toFixed(1)}%`, icon: Eye, tone: "emerald" },
    { id: "avgOrderValue", label: t("dashboard.commandCenter.kpis.avgOrderValue"), value: metrics.averageOrderValue, displayValue: formatCurrency(metrics.averageOrderValue), icon: Banknote, tone: "amber" },
  ];
  ...
  return (
    <section className="relative z-10 mt-4 space-y-3">
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className={`rounded-full border px-3 py-1.5 text-xs font-black ${command.socketConnected ? "border-emerald-300/20 bg-emerald-400/10 text-emerald-100" : "border-amber-300/20 bg-amber-400/10 text-amber-100"}`}>
              {t(command.socketConnected ? "dashboard.commandCenter.connected" : "dashboard.commandCenter.reconnecting")}
            </span>
            <button
              type="button"
            ...
            >
              ...
              {t(command.fullscreen ? "dashboard.commandCenter.exitTvMode" : "dashboard.commandCenter.tvMode")}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5">
        {kpis.map(({ id, ...kpi }) => <RealtimeKPICard key={id} {...kpi} />)}
      </div>
