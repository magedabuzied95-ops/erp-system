    ...
    <div className="rounded-2xl border border-white/10 bg-white/[0.045] p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          ...
          onClick={generate}
          disabled={loading}
          className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-violet-300/20 bg-violet-400/10 px-3 text-xs font-black text-violet-100 disabled:opacity-50"
        >
            ...
            return (
              <div key={suggestion?.id || `${text}-${index}`} className="rounded-xl border border-white/10 bg-slate-950/70 p-3 text-sm leading-6 text-slate-100">
                ...
                <button
                  ...
                  disabled={!text}
                  className="mt-3 h-9 w-full rounded-lg border border-emerald-300/20 bg-emerald-400/10 text-xs font-black text-emerald-100 disabled:opacity-50"
                >
                  Use
                </button>
              </div>
