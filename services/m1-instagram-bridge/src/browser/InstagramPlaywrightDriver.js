...
export class InstagramPlaywrightDriver {
  ...
  async openInbox() {
    ...
    await this.assertExpectedAccount();
    return true;
  }
  async currentAccountUsername() {
    await this.ensurePage();
  ...
  async assertExpectedAccount() {
    const expected = String(this.config.expectedUsername || '').trim().replace(/^@/, '').toLowerCase();
    const current = await this.currentAccountUsername();
    if (current !== expected) {
      throw Object.assign(new Error('Connected Instagram account does not match the configured test account'), {
