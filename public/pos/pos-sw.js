// Shared with the employee-portal and inbox workers. If this fetch fails the
// worker fails to install, which leaves the PREVIOUS worker active -- the safe
// outcome, since the app keeps working rather than installing a worker whose
// image helpers are undefined.
...
const productImages = self.createSwImageCache({
...
self.addEventListener("fetch", (event) => {
  ...
  if (productImages.isImageRequest(request, url)) {
    event.respondWith(productImages.handleFetch(event, request));
    return;
  }
...
self.addEventListener("message", (event) => {
  ...
  if (event.data?.type === "POS_WARM_IMAGES") {
    ...
    event.waitUntil(
      productImages.warm(Array.isArray(event.data.urls) ? event.data.urls : [])
  ...
  if (event.data?.type === "POS_IMAGE_CACHE_STATUS") {
    ...
    event.waitUntil(
      productImages
        .status()
        .then((info) => port?.postMessage({ type: "POS_IMAGE_CACHE_STATUS", ...info }))
