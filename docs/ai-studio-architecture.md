...
| **Restock Intents** (variant-level explicit consent) | new `restock_intents` table + storefront/AI-Studio APIs; recovery now matches exact variant first, legacy wishlist fallback | **Built in Phase 7 / 7.5** — canonical "notify me when THIS variant/size is back"; exact-variant recovery + matchQuality; storefront CTA + AI Inbox + Customer 360 surfaces; internal follow-up ≠ customer_notified. See `docs/restock-intents.md`. |
| **Restock Customer Messaging** (human-approved send) | new `restock_notifications` + approved-send adapter over the **existing** WhatsApp/Meta senders; `messaging.send_customer` stays SENSITIVE | **Built in Phase 8** — first customer-facing action: exact-intent → grounded draft → human Approve/Edit/Reject → confirmed send → `customer_notified_at`. Mode `off`/`preview_only`/`approval_send` (default off); explicit-intent-only; legacy excluded; send-idempotent; NO autonomy. See `docs/restock-customer-messaging.md`. |
| **Messaging Lifecycle & Delivery Reconciliation** | new `message_delivery_events` ledger + `restock_notifications` delivery cols; hooks the **existing** WhatsApp + Meta webhook paths (no new endpoints) | **Built in Phase 9** — provider delivered/read/failed correlated by `provider_message_id`, one canonical monotonic lifecycle (`sent<delivered<read`\|`failed`), persistent event idempotency, out-of-order safe, unmatched-event observability; fixes the Phase 8.5 gap by persisting the confirmed outbound into canonical AI Inbox history; `customer_notified_at` unchanged; no auto-retry; **no inbound→workflow routing**. See `docs/messaging-delivery-reconciliation.md`. |
| **Inbound Omnichannel Intake** (human-approved AI replies) | new `aiInboundIntakeService` + `inbound_ai_mode` (tenant) + `ai_inbound_intake_log`; post-persistence hooks on the **existing** WhatsApp + Meta webhooks; reuses `generateAiInboxReply` (the one brain) + the existing AI Inbox draft + the existing human send | **Built in Phase 10** — inbound TEXT → grounded reply **SUGGESTION** (no send) → human Edit/Reject/Approve & Send in the AI Inbox. Default OFF (global `AI_INBOUND_WORKFLOWS_ENABLED` + tenant `off`/`suggest_only`/`approval_reply`); idempotent (namespaced reply lock); failure-isolated (never breaks the webhook); respects human takeover; `messaging.send_customer` stays SENSITIVE; **no autonomous replies, no second inbox/brain, no inbound multimodal**. See `docs/inbound-omnichannel-intake.md`. |
| **AI Inbox Grounding Quality** (intent accuracy + exact variant) | new `aiInboxGroundingGate` applied inside the existing `generateAiInboxReply`; reuses `productClassificationsService`/`productAliasEngine`/`getInventoryFacts` | **Built in Phase 10.6** — fixes the live failure where "كروكس اسود مقاس 44" was classified GREETING and answered with Air Jordan. Deterministic, failure-isolated: business intent beats greeting; extract product/category/color/size; reject incompatible candidates; claim availability only with exact-variant stock evidence; else clarify. No autonomous replies, no second brain, no writes. See `docs/ai-inbox-grounding-quality.md`. |
| **Unified Footwear Size Grounding** | new `footwearSizeResolver` reusing the ONE canonical `src/shared/lib/crocsSizes.js` table; integrated into `aiInboxGroundingGate` | **Built in Phase 10.7** — customer EU sizes ("44") and Crocs markings ("M10/W12","C10","J5") resolve to exact variants against the product's real sizes, then stock. Deterministic match states (EXACT/UNIQUE/AMBIGUOUS/NO_MAPPING/NO_VARIANT_MATCH); product-type aware; storefront round-trip invariant; availability only with exact-variant evidence. One source of truth, no duplicated tables, no autonomous replies. See `docs/footwear-size-grounding.md`. |
| **Live Assisted AI Inbox Rollout** | per-channel `inbound_ai_channels` + send-side stale guard in the canonical `/conversations/:id/send` + burst debounce + metrics; reuses the existing draft + human send | **Built in Phase 11** — moves inbound AI to LIVE assisted (human-approved) use, staged Messenger→Instagram→WhatsApp. Server-enforced stale protection (newer customer message blocks approving the unedited suggestion), per-channel kill-switchable enablement (tenant mode off = instant kill), process-local burst coalescing (stale = correctness layer), real 24h/7d + per-channel metrics. No autonomous replies, no new sender, no fully_automatic assisted mode. See `docs/assisted-inbox-rollout.md`. |
| **Durable Grounded Product Context (Stage 12.1)** | bounded product-SUBJECT recall in the grounding gate (`resolveConversationProductSubject`); session-scoped, ≤30 min; subject-only, facts re-read fresh | **Phase 12.1** — a continuation that omits the product ("طب مقاس 44؟") reuses the last approved/sent grounded product for that conversation, then re-grounds stock/price/variant from current ERP. Explicit new product wins; ambiguous/expired → clarify; never cross-conversation; style learning has no role in subject selection. See `docs/ai-inbox-durable-product-context.md`. |
| **WhatsApp Assisted (Stage C)** | same pipeline via **Evolution** provider; `inbound_ai_channels.whatsapp` toggle; two autonomous kills always on (`WHATSAPP_AI_AUTO_REPLY=false` + `suggest_only`) | **Phase 13** — the Messenger/Instagram-proven assisted flow for WhatsApp DMs. Inbound = Evolution webhook (already wired to the shared `handleInboundMessageIntake`, fresh non-duplicate only); history-sync never suggests. Identity `whatsapp:<phone>`, never merged. Delivery = image+caption+canonical link (`sendWhatsAppCloudReply` → evolution). Channel-agnostic grounding/durable-context/multi-colour/style; no new brain. See `docs/ai-inbox-whatsapp-assisted-rollout.md`. |
| **Instagram Assisted (Stage B)** | same pipeline; `inbound_ai_channels.instagram` toggle; capability-correct outbound (text + product link — the Meta generic template + image are gated to Messenger only) | **Phase 12** — enables the Messenger-proven assisted flow for Instagram DMs. Channel-agnostic intake/identity (IGSID, never merged)/grounding/stale/A-B/style; the only channel-specific fact is delivery = **text + canonical product link** (no rich card), surfaced to the operator via a delivery chip. Code deployed dormant (Instagram OFF) → owner-run live proof flips it on. No new brain, no autonomous replies. See `docs/ai-inbox-instagram-assisted-rollout.md` + `docs/ai-inbox-assisted-production-runbook.md`. |
