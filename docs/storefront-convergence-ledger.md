...
| `/product/:slug` | Product detail (PDP) | responsive/localization **PASS**; theme **NEEDS_FIX** (CP-7 = PDP must follow global theme) |
...
- Status: PDP responsive + localization **PASS**; **CP-7 pending** (theme convergence). PDP zero-gate not yet met until CP-7 closes.

### CP-7 RECLASSIFIED (explicit user Product/Design ruling — SUPERSEDES the "intentional forced-dark PDP" classification)
- **Ruling**: the PDP MUST follow the global Storefront theme — Light→Light, Dark→Dark. No permanently-dark PDP island in Light. The forced-dark exception is WITHDRAWN. Do NOT extend the forced-dark override; RETIRE it and connect the PDP to the site's existing Light/Dark system.
- **Full ownership traced (current main):** (1) `data-surface-theme="dark"` hardcoded on `StorefrontProductDetailPage.jsx:604` (PDP consumes NO theme value itself); (2) forced-dark block `index.css:6510-6590` keyed on `[data-surface-theme="dark"]` — applies in BOTH themes because the attr is always "dark"; (3) a SEPARATE light-mode compensation layer already exists at `index.css:2815+` (dark text for PDP/checkout cards under `:not(.storefront-dark)`) and canvas light rules at `2524+` — these FIGHT the forced-dark block; (4) PDP JSX is ~875 lines, almost entirely hardcoded-dark inline (~34 `bg-white/[…]`, ~12 `border-white/[…]`, dark hex) with only 2 `dark:` pairs.
- **Minimal fix direction (CSS-mostly, no theme-prop threading needed):** scope the forced-dark block to `.storefront-shell.storefront-dark …` (so it only applies in Dark, reactively via the body class), and ADD a light-surface background block (cards/canvas/chips/CTA) for `:not(.storefront-dark)` mirroring `storefront-light.css` — the existing 2815+ text layer then reads correctly. Verify contrast in Light+Dark × AR+EN + live toggle.
- **STATUS: NOT YET IMPLEMENTED. Held pending a visible browser pane.** Rationale: this is a large theme refactor of a conversion-critical page; my computed-style measurements are internally inconsistent (body read dark in Light despite a `!important` light canvas rule — likely `prefers-color-scheme` interaction) and the acceptance criteria (§12 before/after, §16 four-combo Production verification) genuinely require pixel confirmation, which the currently-hidden pane cannot provide. Executing blind risks shipping a broken PDP. CP-7 code deferred; diagnosis + plan complete.
