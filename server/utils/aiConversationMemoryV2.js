...
export const mergeConversationMemoryV2 = (existingMemory = null, patch = null) => {
  ...
  const result = {
    ...
    activeSize: toText(next.activeSize || next.selectedSize || base.activeSize || base.selectedSize || ""),
    selectedColor: toText(next.selectedColor || base.selectedColor || ""),
    selectedSize: toText(next.selectedSize || base.selectedSize || ""),
    active_size: toText(next.active_size || next.activeSize || next.selectedSize || base.active_size || base.activeSize || base.selectedSize || ""),
    selected_size: toText(next.selected_size || next.selectedSize || next.activeSize || base.selected_size || base.selectedSize || base.active_size || base.activeSize || ""),
...
export const summarizeConversationMemoryV2 = (memory = null) => ({
  ...
  activeSize: toText(memory?.activeSize || memory?.active_size || ""),
  selectedColor: toText(memory?.selectedColor || ""),
  selectedSize: toText(memory?.selectedSize || ""),
  active_size: toText(memory?.active_size || memory?.activeSize || ""),
  selected_size: toText(memory?.selected_size || memory?.selectedSize || ""),
