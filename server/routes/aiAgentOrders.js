...
import { expandProductCardsByColor } from "../services/aiProductColorCarouselService.js";
import {
  ...
  try {
    ...
    if (requestedLines.length) {
      const draft = await createAiOrderDraftLines({
        ...
        customer_secondary_phone: req.body?.customer_secondary_phone || "",
...
router.post("/conversations/:conversationId/reaction", protect, inboxReply(), async (req, res) => {
  try {
    ...
    const instagramRecipientId = envText(
      ...
      conversation.channel_metadata?.instagram_scoped_id ||
...
// One card per colour lives in aiProductColorCarouselService now: the AI auto-reply paths on
// Meta and WhatsApp answer a customer's product photo with the same colour carousel this route
// sends by hand, and two copies of the price/flattening/square-image rules would drift apart.
