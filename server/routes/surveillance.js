...
router.get(
  ...
  permit("surveillance", "view"),
  surveillanceHandler(async (req, res) => {
    const list = await service.listDevices(tid(req), { branchFilter: await branchFilterFor(req) });
    const channels = [];
    for (const device of list) {
      channels.push(...(await devices.listChannels(tid(req), device.id)));
    }
    ...
    res.json({
      ...
      totals: {
        ...
        unknown: list.filter((device) => !["online", "offline"].includes(device.status)).length,
        channels: channels.length,
        channels_enabled: channels.filter((channel) => channel.is_enabled).length,
      },
    });
  }),
);
