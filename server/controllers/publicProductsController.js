...
const normalizeAudienceValue = (value = "") => {
  ...
  if (["men", "man", "male", "mens", "ÿ±ÿ¨ÿßŸÑ", "ÿ±ÿ¨ÿßŸÑŸä"].includes(normalized)) return "men";
  if (["women", "woman", "female", "ladies", "lady", "ŸÜÿ≥ÿßÿ°", "ŸÜÿ≥ÿßÿ¶Ÿä", "ÿ≠ÿ±ŸäŸÖŸä"].includes(normalized)) return "women";
  if (["kids", "kid", "children", "child", "boys", "girls", "ÿßÿ∑ŸÅÿßŸÑ", "ÿ£ÿ∑ŸÅÿßŸÑ", "ÿ∑ŸÅŸÑ"].includes(normalized)) return "kids";
  return "";
};
...
const buildShareAvailablePreviewSvg = ({ req = null, filters = {}, products = [], count = 0 } = {}) => {
  ...
  const title = escapeHtml(`«·„ «Õ »«·„ﬁ«” ${sizeValue}`.trim() || "«·„ «Õ »«·„ﬁ«”");
...
<svg width="1200" height="630" viewBox="0 0 1200 630" fill="none" xmlns="http://www.w3.org/2000/svg" direction="rtl" xml:lang="ar">
  ...
  <text x="92" y="220" fill="#e5e7eb" font-size="24" font-weight="700" font-family="Arial, sans-serif">«› Õ ﬂ· «·„‰ Ã«  «·„ «Õ… «·¬‰ ›Ì M1 Store</text>
  <text x="92" y="254" fill="#cbd5e1" font-size="18" font-weight="700" font-family="Arial, sans-serif">«·„ﬁ«” «·„ «Õ «·¬‰ ÌŸÂ— „»«‘—… œ«Œ· «·„ Ã—</text>
  ${selectedImage
    ? `
      ${imageTag}
    `
...
const renderShareAvailableHtml = ({ req, filters = {}, count = 0, ogImageUrl = "", targetUrl = "", products = [] } = {}) => {
  const sizeLabel = filters.sizes?.length > 1 ? `«·„ «Õ »«·„ﬁ«”«  ${filters.sizes.join("° ")}` : `«·„ «Õ »«·„ﬁ«” ${filters.sizes?.[0] || ""}`.trim();
  const title = escapeHtml(sizeLabel || "«·„ «Õ »«·„ﬁ«”");
  const description = escapeHtml(Number(count || 0) > 0 ? `${count} „ÊœÌ· „ «Õ «·¬‰ ›Ì M1 Store` : "«› Õ «·„‰ Ã«  «·„ «Õ… «·¬‰ ›Ì M1 Store");
  const publicBaseUrl = getPublicAppUrl() || DEFAULT_PUBLIC_APP_URL;
  ...
  const productsPreview = products
    .slice(0, 4)
    .map((product) => `<li>${escapeHtml(firstText(product.name, product.brand_name, product.brand, "·«  ÊÃœ ’Ê—…"))}</li>`)
    .join("");
  return `<!doctype html>
  ...
  <body>
    <main>
      <section class="card">
        ...
        <p style="margin:0 0 18px;opacity:.72;">«·—«»ÿ ÂÌ› Õ «·„‰ Ã«   ·ﬁ«∆Ì« Œ·«· À«‰Ì…. ·Ê „« › Õ‘° «” Œœ„ «·“— »«·√”›·.</p>
        <a href="${fallbackTarget}">› Õ «·„‰ Ã« </a>
...
export const getPublicAvailableOgImage = async (req, res) => {
  ...
  try {
    ...
    const ogImageUrls = products
      ...
      .filter(Boolean);
    const ogTitle = filters.sizes?.length > 1 ? `«·„ «Õ »«·„ﬁ«”«  ${filters.sizes.join("° ")}` : `«·„ «Õ »«·„ﬁ«” ${filters.sizes?.[0] || ""}`.trim();
    const ogDescription = `${ogProductsCount} „ÊœÌ· „ «Õ «·¬‰ ›Ì M1 Store`;
    const ogImageUrl = buildShareAvailableOgImageUrl(req, filters, "png");
    ...
    const svg = buildShareAvailableFallbackSvg({
      req,
      title: filters.sizes?.length ? `«·„ «Õ »«·„ﬁ«” ${filters.sizes.join("° ")}` : "«·„ «Õ »«·„ﬁ«”",
      description: "Ã—¯» „ﬁ«”« √Ê ›· —« ¬Œ—",
      targetUrl,
    });
