...
const sendUnifiedReplyThroughChannelAdapter = async ({ channel = AI_AGENT_CHANNELS.WEB_CHAT, to = "", reply = {}, messageText = "", tenantId = null } = {}) => {
  ...
  if (normalizedChannel === AI_AGENT_CHANNELS.WHATSAPP) {
    ...
    await persistWhatsappTranscriptRow({
      ...
      externalMessageId: result?.results?.[0]?.messages?.[0]?.id || result?.results?.[0]?.message_id || result?.results?.[0]?.messageId || result?.message_id || "",
      providerMessageId: result?.message_id || result?.results?.[0]?.messages?.[0]?.id || result?.results?.[0]?.message_id || result?.results?.[0]?.messageId || "",
