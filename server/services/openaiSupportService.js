...
const FALLBACK_RESPONSE = Object.freeze({
  answer: "„‘ ﬁ«œ— √√ﬂœ «·„⁄·Ê„… „‰ »Ì«‰«  «·„ Ã— œ·Êﬁ Ì. ﬁÊ·¯Ì „ﬁ«”ﬂ √Ê «·„ÊœÌ· «··Ì » œÊ— ⁄·ÌÂ Ê‰ÿ·⁄·ﬂ √ﬁ—» Õ«Ã… „ «Õ….",
  confidence: 0,
...
const formatSuggestedProductPriceAr = (product = {}) => {
  ...
  console.log("[ai-text-price-source]", {
    ...
    text_template: "${top.name} ”⁄—Â ${formatSuggestedProductPriceAr(top)}",
  ...
  return resolved.display_price > 0
    ? `${Number(resolved.display_price).toLocaleString("ar-EG-u-nu-latn")} Ã‰ÌÂ`
    : "«·”⁄— €Ì— „ «Õ Õ«·Ì«";
};

const suggestedProductAvailabilityAr = (product = {}) =>
  Number(product.total_stock || 0) > 0 ? "Ê€«·»« „ «Õ ⁄‰œ‰«" : "Ê„‘ Ÿ«Â— „ «Õ Õ«·Ì«";
...
const buildArabicFirstSalesFallback = ({ message = "", suggestedProducts = [] } = {}) => {
  ...
  if (shouldReplyInArabic(message)) {
    if (!earlyProducts.length) return "» œÊ— ⁄·Ï √Ì ‰Ê⁄ „‰ Ãø ﬁÊ·¯Ì «·„ﬁ«” √Ê «··Ê‰ √Ê «»⁄  ’Ê—…° Ê‰ÿ·⁄ √ﬁ—» Õ«Ã… „ «Õ….";
    const top = earlyProducts[0];
    return `${top.name} ”⁄—Â ${formatSuggestedProductPriceAr(top)}° ${suggestedProductAvailabilityAr(top)}.`;
  }
  const products = normalizeSuggestedProducts(suggestedProducts);
  const hasProducts = products.length > 0;
  const hasOutOfStock = products.some((product) => product.total_stock <= 0 || /out|unavailable|Œ·’|€Ì—/i.test(product.availability));

  if (shouldReplyInArabic(message)) {
    if (!hasProducts) return "ﬁÊ·¯Ì » œÊ— ⁄·Ï „ﬁ«” √Ê ·Ê‰ „⁄Ì‰ø Ê·Ê „⁄«ﬂ ’Ê—… „ÊœÌ· ‰ÿ·⁄·ﬂ ‘»ÂÂ.";
    return [
      "·ﬁÌ  ‘ÊÌ… „ÊœÌ·«  ﬁ—Ì»…. ﬁÊ·¯Ì „ﬁ«”ﬂ √Ê «··Ê‰ «··Ì » Õ»Â Ê‰Ÿ»ÿ·ﬂ «·«Œ Ì«—.",
      hasOutOfStock ? "›ÌÂ „ÊœÌ·«  Ÿ«Â—… »” »⁄÷Â« Œ·’«‰° √ﬁœ— √ÿ·⁄·ﬂ «·„ «Õ »”." : "",
    ].filter(Boolean).join(" ");
  }
    ...
    const response = await getClient().responses.create(
      {
        ...
        instructions: [
          ...
          "Use human Egyptian sales wording like: œÂ ⁄«„· ‘€· Ã«„œ° Œ«„ Â „Õ —„…° «·„ﬁ«” œÂ »ÌŒ·’ »”—⁄…°  Õ» √ÿ·⁄·ﬂ ‘»ÂÂø° œÂ ·«Ìﬁ Ãœ«.",
          "Never describe yourself as an AI assistant or helper.",
          "In Arabic, never say: Ì”⁄œ‰Ì „”«⁄œ ﬂ, ‰⁄ –— ⁄‰ «·≈“⁄«Ã, »—Ã«¡ «·„Õ«Ê·… ·«Õﬁ«, «»⁄ ·Ì, «—”·, ”√”«⁄œﬂ, √‰« Ã«Â“ ··„”«⁄œ….",
          ...
          "Only start order collection when the customer clearly says they want to buy/reserve/order, such as:  „«„ «ÿ·»Â, «ÕÃ“ÂÊ·Ì, «»⁄ ÂÊ·Ì, «⁄„· √Ê—œ—, Â« ·Ì Ê«Õœ,  „«„ Â«ŒœÂ.",
          "When clear buying intent exists, ask for the customer name first exactly in this style:  ‘—›‰« ?? „„ﬂ‰ √⁄—› «”„ Õ÷— ﬂø Then collect phone, address, size/color confirmation, quantity, and notes one step at a time.",
          ...
          "If some suggested products are out of stock, mention naturally in Arabic when relevant: ›ÌÂ „ÊœÌ·«  Ÿ«Â—… »” »⁄÷Â« Œ·’«‰° √ﬁœ— √ÿ·⁄·ﬂ «·„ «Õ »”.",
          "If a product price is missing, null, or 0, do not show 0.00 as a customer-facing price. Say «·”⁄— €Ì— „ «Õ Õ«·Ì« or omit the price.",
