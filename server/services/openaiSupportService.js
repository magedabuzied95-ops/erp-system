...
const FALLBACK_RESPONSE = Object.freeze({
  answer: "مش قادر أأكد المعلومة من بيانات المتجر دلوقتي. قولّي مقاسك أو الموديل اللي بتدور عليه ونطلعلك أقرب حاجة متاحة.",
  confidence: 0,
...
const buildArabicFirstSalesFallback = ({ message = "", suggestedProducts = [] } = {}) => {
  ...
  if (shouldReplyInArabic(message)) {
    if (!earlyProducts.length) return "بتدور على أي نوع منتج؟ قولّي المقاس أو اللون أو ابعت صورة، ونطلع أقرب حاجة متاحة.";
  ...
  if (shouldReplyInArabic(message)) {
    if (!hasProducts) return "قولّي بتدور على مقاس أو لون معين؟ ولو معاك صورة موديل نطلعلك شبهه.";
    return [
      "لقيت شوية موديلات قريبة. قولّي مقاسك أو اللون اللي بتحبه ونظبطلك الاختيار.",
    ...
    const response = await getClient().responses.create(
      {
        ...
        instructions: [
          ...
          "When clear buying intent exists, ask for the customer name first exactly in this style: تشرفنا ❤️ ممكن أعرف اسم حضرتك؟ Then collect phone, address, size/color confirmation, quantity, and notes one step at a time.",
