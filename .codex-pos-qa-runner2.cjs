...
const clickModalAddButton = async (cdp) => evalPage(cdp, () => {
  ...
  if (!target) throw new Error('Add-to-cart button not found');
...
const selectFirstSellerButton = async (cdp, sellerNeedles = []) => evalPage(cdp, (needles) => {
  ...
  const target =
    ...
    buttons.find((button) => {
      const text = String(button.textContent || '').replace(/\s+/g, ' ').trim();
      return text.length <= 4 && !['Open POS shift', 'Create order', 'آجل'].includes(text);
    }) ||
    null;
  if (!target) throw new Error('Seller button not found');
  target.click();
  return { text: String(target.textContent || '').replace(/\s+/g, ' ').trim() };
}, sellerNeedles);
...
(async () => {
  ...
  if (!selectedSellerId) throw new Error('No POS seller user available for the branch');
  ...
  const token = jwt.sign(
    { id: admin.id, role: admin.role || 'admin', tenant_id: admin.tenant_id ?? null, is_super_admin: Boolean(admin.is_super_admin) },
  ...
  await evalPage(cdp, (payload) => {
    ...
    localStorage.setItem('erp.pos.cart', JSON.stringify([]));
    localStorage.setItem('erp.pos.state', JSON.stringify({ paymentMode: 'cash', cashAmount: 0 }));
    localStorage.setItem('pos.lastSalespersonId', payload.sellerId);
  ...
  const shiftButtonState = await evalPage(cdp, () => {
    const buttons = Array.from(document.querySelectorAll('button'));
    const target = buttons.find((button) => {
      const text = String(button.textContent || '').replace(/\s+/g, ' ').trim();
      return text.includes('Open POS shift') || text.includes('فتح الشيفت');
    });
    return target ? { text: String(target.textContent || '').replace(/\s+/g, ' ').trim(), disabled: Boolean(target.disabled) } : null;
  });
  ...
  await waitForPageCondition(cdp, () => {
    ...
    const target = buttons.find((button) => {
      const text = String(button.textContent || '').replace(/\s+/g, ' ').trim();
      return (text.includes('Open POS shift') || text.includes('فتح الشيفت')) && !button.disabled;
    });
    return Boolean(target);
  }, [], 20000);

  const shiftGateLogs = await evalPage(cdp, () => (window.__qaConsoleLogs || []).filter((line) => line.includes('[pos-shift-gate]')));
  if (shiftGateLogs.length) log('shift gate logs', JSON.stringify(shiftGateLogs, null, 2));

  await typeIntoInputByPlaceholder(cdp, ['Opening cash'], '0');
  ...
  await clickVisibleButtonWithText(cdp, 'فتح الشيفت');
  const openShiftRequested = await waitForPageCondition(cdp, () => (window.__qaRequests || []).some((item) => String(item.url || '').includes('/shifts/open') && item.ok), [], 30000)
    .then(() => true)
    .catch(async () => {
      ...
      log('page text after click', (await getText(cdp)).slice(0, 1000));
      return false;
    });
  if (!openShiftRequested) throw new Error('Open shift request did not fire');
  ...
  if (!(openShiftRequestAfter > openShiftRequestBefore)) throw new Error('Open shift request did not fire');
  log('open shift request fired');

  await waitForPageCondition(cdp, () => Boolean(document.querySelector('input[placeholder*="Search products"]') || document.querySelector('input[placeholder*="Search customer by name or phone"]') || String(document.body?.innerText || '').includes('Filters')), [], 60000);
  ...
  await typeIntoInputByPlaceholder(cdp, ['Search products'], 'UNB-M-UNK');
  await clickFirstMatchingCard(cdp, 'UNB-M-UNK');
  await waitForPageCondition(cdp, () => String(document.body?.innerText || '').includes('Barcode Shop') || String(document.body?.innerText || '').includes('Choose the exact color and size'), [], 15000);
  ...
  await typeIntoInputByPlaceholder(cdp, ['Search products'], 'NK-J4-M-MIR-2-BLK-43-2');
  await pressEnterOnInputByPlaceholder(cdp, ['Search products']);
  ...
  await evalPage(cdp, () => {
    ...
    const inc = first?.querySelector('button[aria-label*="Increase quantity"]');
    const dec = first?.querySelector('button[aria-label*="Decrease quantity"]');
  ...
  await evalPage(cdp, () => {
    ...
    const remove = second?.querySelector('button[aria-label*="Remove item"]');
  ...
  await typeIntoInputByPlaceholder(cdp, ['Search customer by name or phone'], customer.phone || customer.name);
  await waitForPageCondition(cdp, () => {
  ...
  await clickVisibleButtonWithText(cdp, 'Create order');
  ...
  if (!cashOrder?.ok) throw new Error(cashOrder?.payload?.message || cashOrder?.error || 'Cash checkout request failed');
  ...
  await clickVisibleButtonWithText(cdp, 'آجل');
  ...
  if (!creditOrder?.ok) throw new Error(creditOrder?.payload?.message || creditOrder?.error || 'Credit checkout request failed');
