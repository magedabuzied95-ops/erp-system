...
const definitions = [
  ...
  ["general.barcode_print_thermal_style", "general", "select", THERMAL_ARTWORK_DEFAULTS.style, "Thermal artwork style", "نمط صورة الثرمال", "Automatic uses the drawing model when it is installed and otherwise picks a filter from how dark the product is. Drawing model redraws the photo as a hand-drawn line drawing inside the system (laces, seams, tread). Illustration traces clean contours and fills only the black accents, line art keeps the photo's detail, halftone simulates greys with dots, outline keeps the edges only, silhouette fills the shape solid.", "تلقائي يستخدم نموذج الرسم إن كان مثبتًا، وإلا يختار فلترًا حسب درجة عتمة المنتج. نموذج الرسم يعيد رسم الصورة كرسم يدوي بالخطوط داخل النظام (الرباط، الخياطة، نقشة النعل). رسم توضيحي يرسم الحدود بخطوط نظيفة ويملأ العناصر السوداء فقط، خطوط تحافظ على تفاصيل الصورة، تدرج نقطي يحاكي الرمادي بالنقاط، حواف تبقي الحدود فقط، ظل يملأ الشكل بالكامل.", { options: [option("auto", "Automatic per product", "تلقائي حسب المنتج"), option("lineart", "Drawing model (local)", "نموذج الرسم (محلي)"), option("sketch", "Illustration", "رسم توضيحي"), option("detail", "Line art", "خطوط"), option("halftone", "Halftone dots", "تدرج نقطي"), option("outline", "Outline only", "حواف فقط"), option("silhouette", "Solid silhouette", "ظل مصمت")], usedBy: ["Barcode Labels", "Products"] }],
