...
import { WHATSAPP_STATUS_LABEL_DEFAULTS } from "./whatsappStatusLabelDefaults.js";
...
const definitions = [
  ...
  ["general.barcode_print_thermal_engine", "general", "select", THERMAL_ARTWORK_DEFAULTS.engine, "Thermal artwork engine", "محرك صورة الثرمال", "Local converts the product photo into black and white artwork inside the system. OpenAI redraws it through an external model and needs a paid API key.", "محلي يحوّل صورة المنتج إلى رسم أبيض وأسود داخل النظام. OpenAI يعيد رسمها عبر نموذج خارجي ويحتاج مفتاح API مدفوع.", { options: [option("local", "Inside the system (no AI)", "داخل النظام (بدون ذكاء اصطناعي)"), option("openai", "OpenAI", "OpenAI")], usedBy: ["Barcode Labels", "Products"] }],
  ["general.barcode_print_thermal_style", "general", "select", THERMAL_ARTWORK_DEFAULTS.style, "Thermal artwork style", "نمط صورة الثرمال", "Automatic uses the illustration service when it is running, then the drawing model when it is installed, and otherwise picks a filter from how dark the product is. Illustration service redraws the product as a clean illustration with Stable Diffusion on this server (slow, best looking). Drawing model traces a hand-drawn line drawing inside the system (laces, seams, tread). Illustration filter traces clean contours and fills only the black accents, line art keeps the photo's detail, halftone simulates greys with dots, outline keeps the edges only, silhouette fills the shape solid.", "تلقائي يستخدم خدمة الرسم إن كانت تعمل، ثم نموذج الرسم إن كان مثبتًا، وإلا يختار فلترًا حسب درجة عتمة المنتج. خدمة الرسم تعيد رسم المنتج كرسم توضيحي نظيف بـ Stable Diffusion على هذا السيرفر (بطيئة، الأجمل شكلًا). نموذج الرسم يرسم رسمًا يدويًا بالخطوط داخل النظام (الرباط، الخياطة، نقشة النعل). فلتر الرسم التوضيحي يرسم الحدود بخطوط نظيفة ويملأ العناصر السوداء فقط، خطوط تحافظ على تفاصيل الصورة، تدرج نقطي يحاكي الرمادي بالنقاط، حواف تبقي الحدود فقط، ظل يملأ الشكل بالكامل.", { options: [option("auto", "Automatic per product", "تلقائي حسب المنتج"), option("diffusion", "Illustration service (Stable Diffusion)", "خدمة الرسم (Stable Diffusion)"), option("lineart", "Drawing model (local)", "نموذج الرسم (محلي)"), option("sketch", "Illustration", "رسم توضيحي"), option("detail", "Line art", "خطوط"), option("halftone", "Halftone dots", "تدرج نقطي"), option("outline", "Outline only", "حواف فقط"), option("silhouette", "Solid silhouette", "ظل مصمت")], usedBy: ["Barcode Labels", "Products"] }],
  ["general.barcode_print_thermal_single_item", "general", "boolean", THERMAL_ARTWORK_DEFAULTS.singleItem, "Thermal artwork shows one shoe", "صورة الثرمال فردة واحدة", "When the photo shows a second shoe standing behind the first, keep only the front one on the label. Photos of a single shoe are left as they are.", "عندما تُظهر الصورة فردة ثانية واقفة خلف الأولى، تُبقي على الأمامية فقط في الملصق. صور الفردة الواحدة لا تتغير.", { usedBy: ["Barcode Labels", "Products"] }],
  ["general.barcode_print_thermal_ink_level", "general", "number", THERMAL_ARTWORK_DEFAULTS.inkLevel, "Thermal ink level", "مستوى الحبر الحراري", "How dark the thermal artwork is burned. 50 is neutral: raise it for pale products, lower it for very dark ones.", "درجة قتامة رسم الثرمال. 50 هو الوضع المتوازن: ارفعه للمنتجات الفاتحة واخفضه للداكنة جدًا.", { validation: { min: 0, max: 100 }, usedBy: ["Barcode Labels", "Products"] }],
  ...
  ["payments.wallet_sms_webhook_secret", "payments", "secret", "", "Vodafone Cash SMS key", "مفتاح رسائل فودافون كاش", "Key the owner's phone sends with each forwarded Vodafone Cash SMS.", "المفتاح اللي موبايل صاحب المحفظة بيبعته مع كل رسالة فودافون كاش.", { isSecret: true, usedBy: ["Payments"] }],
  ...
  // The order's state, written onto the WhatsApp chat itself. Off by default: it puts visible
  // marks on real customer conversations, so it starts only when the shop asks for it.
  ["whatsapp.status_labels", "ai_channels", "json", WHATSAPP_STATUS_LABEL_DEFAULTS, "Order status as a WhatsApp label", "حالة الطلب كليبل واتساب", "Tags the customer's WhatsApp chat with the order's status, so staff holding the phone see it without opening the ERP. Labels are matched by NAME against the labels already created in WhatsApp Business - a name that does not exist there is skipped. `exclusive` removes the previous status label before adding the new one.", "يضع حالة الطلب كليبل على محادثة العميل في واتساب، فيشوفها الموظف من التليفون من غير ما يفتح النظام. الربط بالاسم مع الليبلز الموجودة فعلاً في واتساب بيزنس، وأي اسم غير موجود بيتجاهل. خيار exclusive بيشيل ليبل الحالة السابقة قبل ما يضيف الجديدة.", { usedBy: ["WhatsApp", "Orders", "Shipping"] }],
  ...
  ["orders.bosta_awb_packing_details", "shipping", "boolean", true, "Detailed parcel contents on the airway bill", "تفاصيل محتوى الشحنة على البوليصة", "Send every piece with colour, size and article code in the Bosta description, and print a QR under the label that opens the parcel's models. Off = the old name-only description and a plain label.", "يبعت لبوسطة كل قطعة باللون والمقاس والأرتكل في وصف الشحنة، ويطبع QR تحت البوليصة يفتح صور الموديلات. لو اتقفل يرجع الوصف القديم (اسم الموديل بس) والبوليصة من غير QR.", { usedBy: ["Shipping", "Bosta"] }],
  ...
  ["storefront.free_shipping_threshold", "shipping", "number", 0, "Store-wide free shipping threshold", "حد الشحن المجاني للمتجر كله", "Orders at or above this subtotal ship free. A zone with its own threshold uses that instead. 0 turns it off. The storefront shows a progress bar towards it.", "الطلب اللي إجماليه يوصل للمبلغ ده شحنه مجاني. المنطقة اللي ليها حد خاص بتستخدم حدها. 0 يقفلها. المتجر بيعرض شريط \"باقي كام وتحصل على شحن مجاني\".", { isPublic: true, validation: { min: 0 }, usedBy: ["Storefront Cart", "Checkout", "Merchant Shipping"] }],
  ...
  ["storefront.delivery_estimate_enabled", "shipping", "boolean", true, "Show estimated delivery date", "إظهار تاريخ الوصول المتوقع", "Shows the shopper a real delivery day (\"Arrives Tuesday 15 September\") on the product page and at checkout, from handling time, the zone's transit days, the cut-off time and days off.", "يعرض للعميل يوم وصول حقيقي (\"متوقع وصول طلبك الثلاثاء 15 سبتمبر\") في صفحة المنتج والدفع، محسوب من مدة التجهيز ومدة النقل للمنطقة وآخر موعد للطلب والإجازات.", { usedBy: ["Storefront Product", "Checkout", "AI Support"] }],
  ["storefront.order_cutoff_time", "shipping", "text", "14:00", "Same-day handling cut-off", "آخر موعد لتجهيز الطلب في نفس اليوم", "Orders placed before this time (Cairo) start handling today; later orders start on the next working day.", "الطلب قبل الساعة دي (بتوقيت القاهرة) يبدأ تجهيزه النهارده، وبعدها يبدأ من يوم العمل اللي بعده.", { usedBy: ["Storefront Product", "Checkout"] }],
  ["storefront.shipping_days_off", "shipping", "json", [5], "Shipping days off", "أيام إجازة الشحن", "Weekdays (0 = Sunday … 6 = Saturday) nobody ships or delivers. They are skipped when counting handling and transit days.", "أيام الأسبوع (0 = الأحد … 6 = السبت) اللي مفيش فيها تجهيز ولا توصيل، وبتتخطى في حساب المدة.", { usedBy: ["Storefront Product", "Checkout"] }],
  ["storefront.shipping_holidays", "shipping", "json", [], "Shipping holidays", "إجازات الشحن الرسمية", "Specific dates (YYYY-MM-DD) with no handling or delivery, such as Eid. Skipped like a day off.", "تواريخ محددة (YYYY-MM-DD) مفيش فيها تجهيز ولا توصيل زي الأعياد، وبتتخطى زي يوم الإجازة.", { usedBy: ["Storefront Product", "Checkout"] }],
  ...
  // Price Drop Alert. The website notification follows `enabled`; WhatsApp is a second, separate
  // yes because it messages customers the shop may not have talked to in months.
  ["storefront.price_drop_alert.enabled", "storefront", "boolean", true, "Price drop alerts", "تنبيه نزول السعر", "Customers can follow a product (a button on the product page, or the wishlist) and are told when its price drops.", "العميل يقدر يتابع منتج (زرار في صفحة المنتج أو المفضلة) ويتبلغ لما سعره ينزل.", { isPublic: true, usedBy: ["Storefront Product Page", "Wishlist"] }],
  ["storefront.price_drop_alert.whatsapp_enabled", "storefront", "boolean", false, "Send price drop alerts on WhatsApp", "إرسال تنبيه نزول السعر على واتساب", "Also send the customer a WhatsApp card with the product, the old and new price and a buy button. Sent through the outbound queue, never between 11pm and 10am.", "كمان يتبعت للعميل كارت واتساب فيه المنتج والسعر القديم والجديد وزرار شراء. بيتبعت من طابور الواتساب، ومش بيتبعت من 11 بالليل لـ 10 الصبح.", { usedBy: ["Price Drop Alert"] }],
  ["storefront.price_drop_alert.min_percent", "storefront", "number", 5, "Minimum drop to alert (%)", "أقل نسبة نزول للتنبيه (%)", "A drop smaller than this percentage of the followed price is not announced. 0 announces any drop.", "النزول الأقل من النسبة دي من السعر اللي العميل تابع عليه مش بيتبلّغ. 0 يبلّغ بأي نزول.", { validation: { min: 0, max: 90 }, usedBy: ["Price Drop Alert"] }],
  ...
  ["pos.business_day_start_hour", "pos", "number", 5, "Business day starts at", "اليوم المحاسبي يبدأ الساعة", "The hour the shop's day begins, on a 24-hour clock. The ERP dashboard and the manager portal both run today from this hour to the same hour the next day — 5 means 05:00 to 05:00 — so a night that trades past midnight stays in one day instead of being cut in half at 00:00.", "الساعة اللي يبدأ عندها يوم المحل، بنظام 24 ساعة. الداش بورد وبوابة المدير الاتنين بيحسبوا اليوم من الساعة دي لنفس الساعة تاني يوم — يعني 5 معناها من 5 الصبح لـ 5 الصبح — فالليلة اللي بتعدي نص الليل تفضل في يوم واحد بدل ما تتقطع نصين عند 12.", { validation: { min: 0, max: 23, integer: true } }],
  ...
  ["security.mfa_required", "security", "boolean", false, "Require MFA for admins and Amazon access", "إلزام التحقق الثنائي للمديرين وصلاحيات أمازون", "When on, administrators and anyone holding an amazon.* permission must sign in with an authenticator code; users not yet enrolled are sent to enrolment at login.", "عند التفعيل، يجب على المديرين وأي مستخدم لديه صلاحية أمازون إدخال كود تطبيق المصادقة عند الدخول، ومن لم يفعّله يُطلب منه التفعيل أولاً."],
  ["security.enforce_password_rotation", "security", "boolean", false, "Enforce password expiry and strength at login", "فرض انتهاء وقوة كلمة المرور عند الدخول", "When on, a user whose password is older than the maximum age, does not meet the policy, or was reset by an admin must set a new one before signing in.", "عند التفعيل، المستخدم الذي انتهت صلاحية كلمة مروره أو كانت ضعيفة أو أعاد المدير تعيينها يجب أن يغيّرها قبل الدخول."],
  ["security.password_max_age_days", "security", "number", 365, "Password maximum age (days)", "أقصى عمر لكلمة المرور (أيام)", "Passwords must be changed at least this often.", "يجب تغيير كلمة المرور خلال هذه المدة على الأكثر.", { validation: { min: 30, max: 365, integer: true } }],
