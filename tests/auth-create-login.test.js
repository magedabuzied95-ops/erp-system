...
import { getUsers } from "../server/controllers/usersController.js";
...
test("created user can log in immediately with the same email and password", async () => {
  ...
  const query = async (sql, params = []) => {
    ...
    if (text.includes("SELECT u.id") && text.includes("FROM users u") && text.includes("LEFT JOIN roles r")) {
      ...
      const tenantId = params.length > 1 ? Number(params[1]) : params.length > 0 ? Number(params[0]) : null;
      const rows = state.users
        .filter((user) => tenantId === null || Number(user.tenant_id || 0) === tenantId || user.tenant_id == null)
        .map((user) => ({
          id: user.id,
  ...
  try {
    ...
    if (createdUser) {
      ...
      createdUser.tenant_id ??= 7;
      createdUser.name ??= "New User";
      createdUser.role_id ??= 12;
      createdUser.role ??= "cashier";
      createdUser.is_active ??= true;
      createdUser.is_super_admin ??= false;
    } else {
      state.users.push({
        id: createRes.payload?.user?.id ?? state.nextUserId++,
        tenant_id: 7,
        name: "New User",
        email: "new.user@example.com",
        role_id: 12,
...
test("create user persists to users table, GET /api/users returns it, and login succeeds", async () => {
  const originalQuery = db.query.bind(db);
  const originalConnect = db.connect?.bind(db);
  const state = {
    tenants: [{ id: 7, slug: "acme", name: "Acme Workspace" }],
  ...
  const query = async (sql, params = []) => {
    ...
    if ((text.includes("SELECT\n\n        u.id") || text.includes("SELECT u.id")) && text.includes("LEFT JOIN roles r")) {
      if (text.includes("LOWER(email)")) {
        const email = params[0] ? String(params[0]).toLowerCase() : "";
        const tenantId = params.length > 1 ? Number(params[1]) : null;
        const rows = state.users
          .filter((user) => !email || String(user.email || "").toLowerCase() === email)
          .filter((user) => tenantId === null || Number(user.tenant_id || 0) === tenantId || user.tenant_id == null)
          .map((user) => ({
            id: user.id,
            tenant_id: user.tenant_id,
    ...
    throw new Error(`Unexpected query in create/getUsers/login integration test: ${text.slice(0, 160)}`);
  };
  ...
  try {
    ...
    await getUsers(listReq, listRes);
    assert.equal(listRes.statusCode, 200);
    assert.equal(listRes.payload?.success, true);
    assert.equal(listRes.payload?.users?.length, 1);
    assert.equal(listRes.payload?.users?.[0]?.email, "new.user@example.com");
