...
test("builds the public filtered in-stock storefront link", () => {
  assert.equal(
    ...
    "https://m1store-egy.com/share/available?gender=men&type=sneakers&quality=mirror&size=45&inStock=1"
  );
});
...
test("runs the complete guided Messenger journey and sends the matching link", async () => {
  ...
  assert.match(sent.at(-1).replyText, /gender=men&type=sneakers&quality=mirror&size=45&inStock=1/);
