...
test("storefront mobile navigation uses modern commerce icons", () => {
  ...
  assert.match(navSource, /id: "home"[\s\S]*?icon: ShoppingBag/);
  assert.match(navSource, /id: "categories"[\s\S]*?icon: Footprints/);
  assert.match(navSource, /id: "sale"[\s\S]*?icon: BadgePercent/);
