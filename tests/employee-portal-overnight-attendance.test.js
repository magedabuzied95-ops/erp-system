...
test("checkout by id uses the attendance record date and requires an updated row", async () => {
  const service = await source("server/services/employeePayrollPortalService.js");
  const updateById = service.slice(service.indexOf('"checkout_update_by_id"'), service.indexOf('"checkout_update_by_date"'));
  assert.match(service, /attendanceRow\.attendance_date \|\| attendanceDate/);
  assert.match(updateById, /attendance_date = \$3::date/);
